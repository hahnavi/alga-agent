...
#   - cli.py, alga_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from alga_cli.config import get_config_path
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("ALGA_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ALGA_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ALGA_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like alga-cli)
    ...
    # window). Core Alga tools (toolsets._ALGA_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from alga_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from alga_cli.plugins import get_pre_tool_call_block_message
