...
Storage
-------
Bundles live in ``~/.alga/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ALGA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``alga bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ALGA_HOME.

    Honors ``ALGA_BUNDLES_DIR`` for tests; falls back to
    ``<ALGA_HOME>/skill-bundles``.
    """
    override = os.environ.get("ALGA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_alga_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `alga bundles` CLI subcommand.
# ---------------------------------------------------------------------------
