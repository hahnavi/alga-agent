...
``auth.json``.  They intentionally have no dependency on ``alga_cli.auth`` so
...
# Sources Alga owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Alga-owned OAuth/device-code state)
