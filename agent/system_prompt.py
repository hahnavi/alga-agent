...
upstream prefix cache warm.  See ``alga-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Alga never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ALGA_HOME persona while keeping
    ...
    # Pointer to the alga-agent skill + docs for user questions about Alga itself.
    stable_parts.append(ALGA_AGENT_HELP_GUIDANCE)
    ...
    # ALGA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Alga profile the agent is running
    # under so it doesn't conflate ~/.alga/skills/ (default profile) with
    # ~/.alga/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Alga profile: default. Other profiles (if any) live "
            "under ~/.alga/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Alga profile: {active_profile}. This session reads "
            f"and writes ~/.alga/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.alga/skills/, ~/.alga/plugins/, "
            f"~/.alga/cron/, ~/.alga/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the alga-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Alga never rebuilds or reinjects parts of it
