...
Plugins built on Alga Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Alga
...
:class:`~alga_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Alga' main config
...
handles every provider, fallback chain, and per-task override Alga
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`alga_cli.plugins.PluginContext`
