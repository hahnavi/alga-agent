...
projection into Alga' display, approval bridging, transcript projection into
...
"codex_app_server"`. Alga' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ALGA_KANBAN_TASK"):
            kanban_db = spawn_env.get("ALGA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ALGA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ALGA_HOME", os.path.expanduser("~/.alga")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "alga",
        client_title: str = "Alga Agent",
