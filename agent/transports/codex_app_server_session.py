...
Owns one Codex thread per Alga session. Drives `turn/start`, consumes
...
# Alga' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ALGA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Alga/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Alga session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ALGA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ALGA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="alga",
            client_title="Alga Agent",
            client_version=_get_alga_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Alga' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Alga' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own alga-tools callback we
            # auto-accept — the user already approved Alga' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Alga approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Alga returns 'once', 'session', 'always', or 'deny'.
...
def _get_alga_version() -> str:
    """Best-effort Alga version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("alga-agent")
