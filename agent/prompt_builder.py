...
from alga_constants import get_alga_home, get_skills_dir, is_wsl
...
_ALGA_MD_NAMES = (".alga.md", "ALGA.md")


def _find_alga_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.alga.md`` or ``ALGA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Alga Agent, an intelligent AI assistant created by Nous Research. "
...
ALGA_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Alga Agent "
    "itself, load the `alga-agent` skill with skill_view(name='alga-agent') "
    "before answering. Docs: https://alga-agent.hahnavi.my.id/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.alga/kanban.db`. Your task id is in "
    "`$ALGA_KANBAN_TASK`; your workspace is `$ALGA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ALGA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `alga kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Alga WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Alga itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Alga runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Alga itself is running. The host OS, home, and cwd "
                f"of the Alga process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Alga "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Alga
    ...
    extra = (os.getenv("ALGA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_alga_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.alga/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ALGA_PLATFORM")
        or get_session_env("ALGA_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Alga Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `alga-agent` skill "
            "first. It has the actual commands (e.g. `alga config set …`, `alga tools`, "
            "`alga setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: alga setup, alga setup tools, alga setup terminal, alga status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ALGA_HOME and return its content, or None.
    ...
    """
    try:
        from alga_cli.config import ensure_alga_home
        ensure_alga_home()
    except Exception as e:
        logger.debug("Could not ensure ALGA_HOME before loading SOUL.md: %s", e)

    soul_path = get_alga_home() / "SOUL.md"
...
def _load_alga_md(cwd_path: Path) -> str:
    """.alga.md / ALGA.md — walk to git root."""
    alga_md_path = _find_alga_md(cwd_path)
    if not alga_md_path:
        return ""
    try:
        content = alga_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = alga_md_path.name
        try:
            rel = str(alga_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".alga.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", alga_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .alga.md / ALGA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ALGA_HOME is independent and always included when present.
    ...
    # SOUL.md from ALGA_HOME only — skip when already loaded as identity
    if not skip_soul:
