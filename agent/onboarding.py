...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Alga starts and finds ``~/.openclaw/``.

    Points users at ``alga claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``alga claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Alga, run "
        "`alga claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `alga claw cleanup` (renames it to ~/.openclaw.pre-migration — "
