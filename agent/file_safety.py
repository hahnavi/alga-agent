...
def _alga_home_path() -> Path:
    """Resolve the active ALGA_HOME (profile-aware) without circular imports."""
    try:
        from alga_constants import get_alga_home  # local import to avoid cycles
        return get_alga_home()
    except Exception:
        return Path(os.path.expanduser("~/.alga"))


def _alga_root_path() -> Path:
    """Resolve the Alga root dir (always the parent of any profile, never per-profile)."""
    try:
        from alga_constants import get_default_alga_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    alga_home = _alga_home_path()
    alga_root = _alga_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(alga_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(alga_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ALGA_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ALGA_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Alga control-plane files: block both the ACTIVE profile's view
    # (alga_home) AND the global root view. Without the root pass, a
    ...
    alga_dirs = []
    for base in (_alga_home_path(), _alga_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in alga_dirs:
                alga_dirs.append(real)
        except Exception:
            continue

    for base_real in alga_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Alga path.

    Three categories are blocked:

      * Internal Alga cache files under ``ALGA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ALGA_HOME and the global Alga
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.alga/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ALGA_HOME (profile-aware) AND the global
    # Alga root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ALGA_HOME points at
    ...
    for hd in alga_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Alga cache file "
    ...
    for hd in alga_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Alga credential store "
    ...
    for hd in alga_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Alga MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Alga MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Alga profiles are separate ALGA_HOME dirs under
...
#
# Reference: May 2026 incident where a alga-security profile session
# edited skills under both ``~/.alga/profiles/alga-security/skills/``
# AND ``~/.alga/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ALGA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ALGA_HOME.
    ...
    """
    try:
        home_real = _alga_home_path().resolve()
        root_real = _alga_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Alga scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Alga entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Alga profile {info['target_profile']!r}, but the "
