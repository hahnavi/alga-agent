...
as ``kind: backend``) or ``~/.alga/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``alga tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ALGA_HOME/cache/videos/``, creating parents as needed."""
    from alga_constants import get_alga_home

    path = get_alga_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ALGA_HOME/cache/videos/``.
