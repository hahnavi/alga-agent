...
from alga_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ALGA_PLATFORM`` or
            ``ALGA_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``alga`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.alga/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``alga`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ALGA_HOME, not cwd
        if not p.is_absolute():
            p = (alga_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.alga/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    alga = metadata.get("alga") or {}
    if not isinstance(alga, dict):
        alga = {}
    return {
        "fallback_for_toolsets": alga.get("fallback_for_toolsets", []),
        "requires_toolsets": alga.get("requires_toolsets", []),
        "fallback_for_tools": alga.get("fallback_for_tools", []),
        "requires_tools": alga.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Alga metadata, VCS, virtualenv/dependency, and cache
