...
def _state_file() -> Path:
    return get_alga_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.alga/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `alga update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``alga update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``alga curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `alga curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Alga' background skill CURATOR. This is an "
    ...
    "into ~/.alga/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: alga-config-*, alga-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.alga/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.alga/logs/curator/``)
    ...
    authored skill data in ``~/.alga/skills/``.

    ``ensure_alga_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_alga_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every alga install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: alga curator status
        keep an umbrella stable: alga curator pin document-tools
    ...
    lines.append("full report: alga curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: alga curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.alga/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.alga/skills/.archive/` for "
            "safety and can be restored via `alga curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.alga/skills/.archive/`. "
            "Restore any via `alga curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `alga curator restore <name>`")
    lines.append("- All archives live under `~/.alga/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `alga curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `alga update` only if "
        ...
        # recorded in state so `alga curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``alga model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `alga model` → auxiliary
