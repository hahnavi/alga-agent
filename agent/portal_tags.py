...
Every Alga request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Alga Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=alga-agent",
        "client=alga-client-v<__version__>",
    ]

The version is sourced live from ``alga_cli.__version__`` so it auto-aligns
...
``alga_cli.__version__`` is the canonical source of truth.
"""
...
def _alga_version() -> str:
    """Return the current Alga release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``alga_cli`` cannot be imported (should
...
def alga_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=alga-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=alga-client-v{_alga_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=alga-agent", alga_client_tag()]
