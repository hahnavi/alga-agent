...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - alga_home (str): The active ALGA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.alga``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "alga").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'alga memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], alga_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'alga memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``alga_home`` is the active ALGA_HOME directory path.
