"""Windows UTF-8 bootstrap for Alga entry points.
...
should be imported at the very top of every Alga entry point
(``alga``, ``alga-agent``, ``alga-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import alga_bootstrap``
# (or ``from alga_bootstrap import apply_windows_utf8_bootstrap``) at
