"""
Dump command for alga CLI.

Outputs a compact, plain-text summary of the user's Alga setup
...
from alga_cli.config import get_alga_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.alga_build_sha`` by the Dockerfile's
    ``ALGA_GIT_SHA`` build-arg (see ``alga_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``alga dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from alga_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(alga_home: Path) -> int:
...
def _cron_summary(alga_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = alga_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"alga_home:      {display_alga_home()}")
    ...
    toolsets = config.get("toolsets", ["alga-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(alga_home)}")
    lines.append(f"  skills:             {_count_skills(alga_home)}")
