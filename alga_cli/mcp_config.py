"""
MCP Server Management CLI — ``alga mcp`` subcommand.

Implements ``alga mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.alga/config.yaml under the ``mcp_servers`` key.
"""
...
from alga_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_alga_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from alga_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``alga mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import AlgaTokenStorage
        return AlgaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # alga_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  alga mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  alga mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  alga mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_alga_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: alga mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_alga_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `alga mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  alga mcp add <name> --url <endpoint>')
        _info('  alga mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `alga mcp remove` + `alga mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `alga mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'alga mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``alga mcp`` subcommands."""
    ...
    if action == "picker":
        from alga_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from alga_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from alga_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `alga plugin`.
        from alga_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("alga mcp                                    Open the catalog picker (default)")
        _info("alga mcp catalog                            List Nous-approved MCPs")
        _info("alga mcp install <name>                     Install a catalog MCP")
        _info("alga mcp serve                              Run as MCP server")
        _info("alga mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("alga mcp add <name> --command <cmd>         Add a stdio server")
        _info("alga mcp add <name> --preset <preset>       Add from a known preset")
        _info("alga mcp remove <name>                      Remove a server")
        _info("alga mcp list                               List configured servers")
        _info("alga mcp test <name>                        Test connection")
        _info("alga mcp configure <name>                   Toggle tools")
        _info("alga mcp login <name>                       Re-authenticate OAuth")
        print()
