"""
Doctor command for alga CLI.

Diagnoses issues with Alga Agent setup.
"""
...
from alga_cli.config import get_project_root, get_alga_home, get_env_path
from alga_cli.env_loader import load_alga_dotenv
from alga_constants import display_alga_home

PROJECT_ROOT = get_project_root()
ALGA_HOME = get_alga_home()
_DHH = display_alga_home()  # user-facing display path (e.g. ~/.alga or ~/.alga/profiles/coder)

# Load environment variables from ~/.alga/.env so API key checks work
_env_path = get_env_path()
load_alga_dotenv(alga_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from alga_cli.colors import Colors, color
from alga_cli.models import _ALGA_USER_AGENT
from alga_constants import OPENROUTER_MODELS_URL
...
from alga_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.alga/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ALGA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from alga_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from alga_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches alga_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``alga --version`` reporting a stale version while
    ...
    """
    try:
        from alga_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != alga_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'alga update', or set "
            "alga_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-alga and dashboard static services are up
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `alga profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from alga_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `alga`.
    os.environ.setdefault("ALGA_INTERACTIVE", "1")

    # Handle `alga doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.alga/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Alga Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`alga doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and alga_cli/__init__.py versions
    ...
    # Check ~/.alga/.env (primary location for user config)
    env_path = ALGA_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'alga setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'alga setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'alga setup' to create one")
                issues.append("Run 'alga setup' to create .env")
    
    # Check ~/.alga/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from alga_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from alga_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'alga config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from alga_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.alga/.env or run 'alga setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'alga setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'alga config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from alga_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from alga_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'alga setup' to migrate config")
                else:
                    issues.append("Run 'alga doctor --fix' or 'alga setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'alga doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Alga' own device-code flow — the Codex CLI is
    ...
    if alga_home.exists():
        ...
        alga_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = alga_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Alga a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Alga Agent Persona\n\n"
                "<!-- Edit this file to customize how Alga communicates. -->\n\n"
                "You are Alga, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = alga_home / "memories"
    ...
    state_db_path = alga_home / "state.db"
    ...
    wal_path = alga_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'alga doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "alga"
        ...
        _cmd_link = _cmd_link_dir / "alga"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(alga not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/alga → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/alga points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/alga → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/alga — run 'alga doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/alga exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/alga not found",
                    "(alga command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/alga → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/alga symlink — run 'alga doctor --fix'")
    ...
    try:
        from alga_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `alga doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'alga config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``alga doctor``.
    ...
    # never the legitimate source for `alga doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'alga setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ALGA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: alga skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ALGA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: alga memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: alga memory setup",
                    "No Honcho API key — run 'alga memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run alga memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: alga memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: alga memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from alga_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "alga -p" in content:
                            _m = _re.search(r"alga -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'alga doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
