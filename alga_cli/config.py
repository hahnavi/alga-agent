"""
Configuration management for Alga Agent.

Config files are stored in ~/.alga/ for easy access:
- ~/.alga/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.alga/.env         - API keys and secrets

This module provides:
- alga config          - Show current configuration
- alga config edit     - Open config in editor
- alga config set      - Set a specific value
- alga config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.alga/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``alga logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  alga config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Alga makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Alga itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``alga update``, the TUI build.
...
#   on every plugin install / ``alga update``.
...
# * ``ALGA_HOME`` / ``ALGA_PROFILE`` / ``ALGA_CONFIG`` /
#   ``ALGA_ENV`` — Alga runtime location flags. Writing these into
...
#
# IMPORTANT: ``ALGA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ALGA_GEMINI_CLIENT_ID,
# ALGA_LANGFUSE_PUBLIC_KEY, ALGA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Alga runtime location — never via dashboard env writer.
    # NOT a ALGA_* blanket: integration credentials (ALGA_GEMINI_*,
    # ALGA_LANGFUSE_*, ALGA_SPOTIFY_*, ...) ARE allowed.
    "ALGA_HOME", "ALGA_PROFILE", "ALGA_CONFIG", "ALGA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Alga runtime location "
            "(ALGA_HOME, ALGA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `alga plugins enable
    # observability/langfuse` or `alga tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ALGA_LANGFUSE_ENV",
    "ALGA_LANGFUSE_RELEASE",
    "ALGA_LANGFUSE_SAMPLE_RATE",
    "ALGA_LANGFUSE_MAX_CHARS",
    "ALGA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from alga_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ALGA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_alga_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Alga is running in package-manager-managed mode.

    Two signals: the ALGA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ALGA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade alga-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Alga was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.alga/.install_method`` file (written by installers)
    2. ALGA_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``hahnavi/alga-agent`` image stamps ``docker``
    ...
    so ``alga update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_alga_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.alga/.install_method."""
    stamp = get_alga_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Alga lives in a ``uv tool`` layout.

    ``uv tool install alga-agent`` places the install at
    ``.../uv/tools/alga-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``alga-agent`` happens to be uv-tool-installed on the machine while
    the *active* Alga is a regular pip/venv install, causing
    ``alga update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/alga-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade alga-agent"
    if method == "docker":
        return "docker pull hahnavi/alga-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade alga-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade alga-agent"
        return "pip install --upgrade alga-agent"
    return "alga update"
...
# Long-form text for ``alga update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# alga_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Alga, it doesn't update
...
✗ ``alga update`` doesn't apply inside the Docker container.

Alga Agent runs as a published image (hahnavi/alga-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull hahnavi/alga-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate alga-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm hahnavi/alga-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/hahnavi/alga-agent/tags
  • Your config and session history live under ``$ALGA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``alga update`` inside Docker.
...
def format_managed_message(action: str = "modify this Alga installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ALGA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Alga installation is managed by NixOS "
            f"(ALGA_MANAGED={env_hint}).\n"
            "Edit services.alga-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Alga installation is managed by Homebrew "
            f"(ALGA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade alga-agent"
        )

    return (
        f"Cannot {action}: this Alga installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Alga."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ALGA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, alga_bin
    or None if container mode is not active, we're already inside the
    container, or ALGA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ALGA_DEV") == "1":
        return None
    ...
    container_mode_file = get_alga_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "alga-agent")
    exec_user = info.get("exec_user", "alga")
    alga_bin = info.get("alga_bin", "/data/current-package/bin/alga")
...
# Re-export from alga_constants — canonical definition lives there.
from alga_constants import get_alga_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_alga_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_alga_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_alga_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ALGA_UID / ALGA_GID env vars set by Docker deployments.

    Docker containers running Alga commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ALGA_HOME once, but
    subdirectories created at runtime by ``ensure_alga_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ALGA_UID", "").strip()
    gid_str = os.environ.get("ALGA_GID", "").strip()
    try:
...
def _chown_to_alga_uid(path) -> None:
    """Chown ``path`` to ``ALGA_UID:ALGA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_alga_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_alga_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the alga group can
    share state with the gateway service.

    The mode can be overridden via the ALGA_HOME_MODE environment variable
    (e.g. ALGA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ALGA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ALGA_UID``/``ALGA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ALGA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_alga_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Alga runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ALGA_CONTAINER") or os.environ.get("ALGA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ALGA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ALGA_HOME if the user doesn't have one yet."""
...
def ensure_alga_home():
    """Ensure ~/.alga directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_alga_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_alga_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ALGA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Alga-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Alga
        ...
        # identity slot (SOUL.md). Empty by default. The ALGA_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Alga auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Alga sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Alga
        ...
        # bundled Alga image, which drops to the `alga` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Alga sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Alga should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``alga chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.alga/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: alga sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Alga truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``alga kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``alga kanban decompose`` and the kanban
        ...
        # ``alga profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `alga model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `alga --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `alga -c` muscle memory.  Default off so existing
        # users aren't surprised.  ALGA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `alga --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/alga`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.alga/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.alga/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Alga feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Alga auto-pauses the goal and
    ...
    # always goes to ~/.alga/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ALGA_SKILL_DIR} and ${ALGA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `alga curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `alga update`, so pruning them only
        ...
        # skipped), snapshot ~/.alga/skills/ into
        # ~/.alga/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `alga curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for alga-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "🍀 *Alga Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ALGA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.alga/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ALGA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `alga doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``alga_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Alga to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ALGA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `alga -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `alga` launches with
        ...
        # decomposition is manual via `alga kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     alga-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Alga tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.alga/logs/.
    ...
    # update model picker lists without shipping a alga-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://alga-agent.hahnavi.my.id/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `alga model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.alga/.env,
        ...
        # behavior: files must live under the Alga cache, under
        ...
        # user. Bridged to ALGA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Alga
        # cache (~/.alga/cache/{documents,images,audio,video,screenshots})
        ...
        # to ALGA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ALGA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ALGA_MEDIA_TRUST_RECENT_SECONDS.
        # Only consulted when ``strict`` is true.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.alga/state.db.
    ...
    "sessions": {
        ...
        # default of ``alga sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.alga/sessions/session_{sid}.json`` on every turn
    ...
    # ``alga update`` behaviour.
    "updates": {
        # Run a full ``alga backup``-style zip of ALGA_HOME before every
        # ``alga update``.  Backups land in ``<ALGA_HOME>/backups/`` and
        # can be restored with ``alga import <path>``.  Off by default —
        # on large ALGA_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ALGA_HOME>/lsp/bin/`` on first use.
    ...
    # `alga tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.alga/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.alga/.env (or your shell) and never in config.yaml.
            ...
            # ~/.alga/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'alga model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Alga auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @alga:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ALGA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #alga)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: alga-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'alga-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Alga API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Alga API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Alga API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ALGA_TOOL_PROGRESS and ALGA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.alga.config`` entries, then checks
    ...
    try:
        ...
        # should never break `alga update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Alga versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``alga_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'alga setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Alga won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'alga doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ALGA_HOME", "~/.alga")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ALGA_TOOL_PROGRESS")
            old_mode = get_env_value("ALGA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ALGA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ALGA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ALGA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ALGA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ALGA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_alga_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `alga plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `alga curator status` has no
    ...
    #   3. Creates `~/.alga/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_alga_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_alga_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: alga config set <key> <value>")
    ...
    # metadata.alga.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _alga_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.alga/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.alga/config.yaml.
    ...
    (which change ``ALGA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — alga auth) — OpenAI Codex
#   nous         (OAuth — alga auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.alga/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.alga/.env.
    ...
    interactive menu render (`alga tools`, `alga setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `alga tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Alga env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.alga/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.alga/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.alga/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.alga/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Alga — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Alga vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.alga/.env or environment."""
...
def show_config():
    ...
    print(color("│              🍀 Alga Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  alga config edit     # Edit config file", Colors.DIM))
    print(color("  alga config set <key> <value>", Colors.DIM))
    print(color("  alga setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: alga config set <key> <value>")
            print()
            print("Examples:")
            print("  alga config set model anthropic/claude-sonnet-4")
            print("  alga config set terminal.backend docker")
            print("  alga config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'alga config migrate' to add them")
        ...
        print("  alga config           Show current configuration")
        print("  alga config edit      Open config in editor")
        print("  alga config set <key> <value>   Set a config value")
        print("  alga config check     Check for missing/outdated config")
        print("  alga config migrate   Update config with new options")
        print("  alga config path      Show config file path")
        print("  alga config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``alga config`` UI so users
