...
- ``alga_cli.providers``        -- canonical provider identity + overlays
- ``alga_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_alga_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_ALGA_NON_AGENTIC_RE.search(model_name))


def _check_alga_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_alga_non_agentic(model_name):
        return _ALGA_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``alga config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from alga_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'alga model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'alga doctor' — config issues detected:"
    ...
    # Mirrors the same logic in alga_cli.runtime_provider.resolve_runtime_provider;
    ...
    alga_warn = _check_alga_model_warning(new_model)
    if alga_warn:
        warnings.append(alga_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from alga_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from alga_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by alga provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://alga-agent.hahnavi.my.id/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Alga release. Falls back to the in-repo
    ...
    # --- 1. Check Alga-mapped providers ---
    for alga_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(alga_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ALGA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from alga_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(alga_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `alga model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(alga_id)
        if not model_ids:
            model_ids = curated.get(alga_id, [])
            if alga_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(alga_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Alga-only providers (nous, openai-codex, copilot, opencode-go) ---
    from alga_cli.providers import ALGA_OVERLAYS
    from alga_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Alga provider ID.
    # ALGA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Alga IDs ("copilot").
    _mdev_to_alga = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ALGA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Alga slug — e.g. "github-copilot" → "copilot"
        alga_slug = _mdev_to_alga.get(pid, pid)
        if alga_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(alga_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, alga_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or alga_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(alga_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", alga_slug, exc)
        ...
        # configured.
        if not has_creds and alga_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_alga_oauth_credentials,
                )
                alga_creds = read_alga_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (alga_creds and alga_creds.get("accessToken")) or \
        ...
        if alga_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(alga_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(alga_slug)
                model_ids = _ids if _ids else (curated.get(alga_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(alga_slug, []) or curated.get(pid, [])
        elif alga_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `alga model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from alga_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(alga_slug, []) or curated.get(pid, [])
                if alga_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(alga_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": alga_slug,
            "name": get_label(alga_slug),
            "is_current": alga_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "alga",
        ...
        seen_slugs.add(alga_slug.lower())
        _record_builtin_endpoint(alga_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ALGA_OVERLAYS (keeps /model in sync
    # with `alga model`).
    try:
        from alga_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Alga's canonical write key (matches
            ...
            # Alga writes ``models:`` as a dict keyed by model id
            # (see alga_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Alga's own writer uses; a hyphen variant is accepted
            ...
            # active model. Alga's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, alga_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`alga_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from alga_cli.models import fetch_openrouter_models
