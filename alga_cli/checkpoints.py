"""`alga checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.alga/checkpoints/``.  Actions:

    alga checkpoints               # same as `status`
    alga checkpoints status        # total size, project count, breakdown
    alga checkpoints list          # per-project checkpoint counts + workdir
    alga checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    alga checkpoints clear [-f]    # nuke the entire base (asks first)
    alga checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    alga checkpoints
    alga checkpoints prune --retention-days 3 --max-size-mb 200
    alga checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: alga checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``alga checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `alga checkpoints` → status
