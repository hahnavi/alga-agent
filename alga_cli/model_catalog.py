...
The Alga docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.alga/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/hahnavi/alga-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_alga_home() / "cache" / "model_catalog.json"
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``alga model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
