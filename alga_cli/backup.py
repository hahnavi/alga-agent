"""
Backup and import commands for alga CLI.

`alga backup` creates a zip archive of the entire ~/.alga/ directory
(excluding the alga-agent repo and transient files).

`alga import` restores from a backup zip, overlaying onto the current
ALGA_HOME root.
"""
...
from alga_constants import get_default_alga_root, get_alga_home, display_alga_home
...
_EXCLUDED_DIRS = {
    "alga-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to alga root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ALGA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Alga home directory."""
    alga_root = get_default_alga_root()

    if not alga_root.is_dir():
        print(f"Error: Alga home directory not found at {alga_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"alga-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"alga-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_alga_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(alga_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(alga_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(alga_root)
    ...
    print(f"\nRestore with: alga import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Alga backup.
    ...
    # Look for telltale files that a alga home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Alga backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.alga/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a alga dir name
        if prefix in {".alga", "alga"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_alga_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (alga_root / "config.yaml").exists()
        has_env = (alga_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Alga configuration.")
        ...
        for member in members:
            ...
            target = alga_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(alga_root.resolve())
            except ValueError:
        ...
        profiles_dir = alga_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # alga_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: alga profile list  (after installing alga)")

        # Guidance
        print()
        if not (alga_root / "alga-agent").is_dir():
            print("Note: The alga-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: alga update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  alga -p {pname} gateway install")

        print("Done. Your Alga configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and alga backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ALGA_HOME).
...
# — `alga update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(alga_home: Optional[Path] = None) -> Path:
    home = alga_home or get_alga_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    alga_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(alga_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ALGA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``alga update``.
    ...
    Args:
        ...
        alga_home: Override for the Alga home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(alga_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for alga backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_alga_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, alga_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``alga_root`` to ``out_path``.
...
def _pre_update_backup_dir(alga_home: Optional[Path] = None) -> Path:
    home = alga_home or get_alga_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ALGA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ALGA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``alga update``) should continue even if the backup fails.
    """
    alga_root = alga_home or get_default_alga_root()
    if not alga_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(alga_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, alga_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `alga claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ALGA_HOME under ``backups/`` before a
    ``alga claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``alga import <archive>``.  Writes to
    ``<ALGA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `alga import` and the
