"""alga hooks — inspect and manage shell-script hooks.

Usage::

    alga hooks list
    alga hooks test <event> [--for-tool X] [--payload-file F]
    alga hooks revoke <command>
    alga hooks doctor

Consent records live under ``~/.alga/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.alga/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``alga hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: alga hooks {list|test|revoke|doctor}")
        print("Run 'alga hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.alga/config.yaml.")
        print("See `alga hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `alga hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `alga hooks test` and `alga hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Alga wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `alga hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `alga hooks doctor` would execute
