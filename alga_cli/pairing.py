...
Usage:
    alga pairing list              # Show all pending + approved users
    alga pairing approve <platform> <code>  # Approve a pairing code
    alga pairing revoke <platform> <user_id> # Revoke user access
    alga pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle alga pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: alga pairing {list|approve|revoke|clear-pending}")
        print("Run 'alga pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.alga/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'alga pairing list' to see pending codes.\n")
