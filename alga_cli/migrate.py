"""CLI handlers for ``alga migrate ...``.

Currently exposes only ``alga migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``alga migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: alga migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `alga migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `alga doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
