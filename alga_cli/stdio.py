...
on Japanese locales, etc.).  Alga's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``alga_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Alga is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ALGA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ALGA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Alga-managed Git install directories so
    ...
    # already-running shells don't see the change — which means alga
    ...
    # install without Alga-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Alga's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Alga's
    ``alga config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Alga (User env var in Windows Settings, or
    export in a PowerShell profile) and Alga picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Alga-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\alga\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``alga`` from right after install) retain their old PATH.

    Any subprocess Alga spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Alga-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "alga", "git", "cmd"),
        os.path.join(local_appdata, "alga", "git", "bin"),
        os.path.join(local_appdata, "alga", "git", "usr", "bin"),
        # Alga venv Scripts directory — host of the alga.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes alga, but harmless to include.
        os.path.join(local_appdata, "alga", "alga-agent", "venv", "Scripts"),
