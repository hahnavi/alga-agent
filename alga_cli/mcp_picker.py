"""MCP picker — interactive `alga mcp picker` (also the default `alga mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``alga mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `alga plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from alga_cli.cli_output import prompt_yes_no
from alga_cli.curses_ui import curses_single_select
from alga_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Alga session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from alga_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `alga mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: alga mcp install <name>    Picker: alga mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Alga is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Alga — run `alga update` "
...
def show_catalog() -> None:
    """`alga mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`alga mcp picker` (and default `alga mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`alga mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `alga mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
