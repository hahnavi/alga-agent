"""
Interactive setup wizard for Alga Agent.
...
from alga_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://alga-agent.hahnavi.my.id/docs"
...
# display_alga_home imported lazily at call sites (stale-module safety during alga update)
...
from alga_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from alga_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🍀 Alga Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Alga using environment variables or config commands:")
    print_info("  alga config set model.provider custom")
    print_info("  alga config set model.base_url http://localhost:8080/v1")
    print_info("  alga config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'alga setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from alga_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'alga setup')")


def _print_setup_summary(config: dict, alga_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'alga setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from alga_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `alga tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from alga_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'alga setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'alga setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'alga setup terminal'"))
    ...
    # Spotify (OAuth via alga auth spotify — check auth.json, not env vars)
    try:
        from alga_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'alga setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {alga_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('alga setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('alga setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('alga setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('alga setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('alga setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('alga config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('alga config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('alga config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('alga', Colors.GREEN)}              Start chatting")
    print(f"   {color('alga gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('alga doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `alga tools` and `alga setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``alga model``)
    ...
    provider added to ``alga model`` is automatically available here.
    ...
    # Delegate to the shared alga model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from alga_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: alga model")
    ...
    # on demand via `alga auth add`, `alga setup` vision, and
    # `alga setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``alga model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from alga_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.alga/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Alga auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via alga setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'alga setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Alga runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `alga setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `alga setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ALGA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ALGA_MAX_ITERATIONS")
    ...
    print_info("  Run `alga setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ALGA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Alga delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `alga update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://alga-agent.hahnavi.my.id/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ALGA_HOME, and print
    ...
    """
    try:
        from alga_cli.slack_cli import _build_full_manifest
        from alga_constants import get_alga_home

        manifest = _build_full_manifest(
            bot_name="Alga",
            bot_description="Your Alga agent on Slack",
        )
        target = Path(get_alga_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `alga slack manifest --write` anytime to refresh after "
            "Alga adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Alga delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Alga to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from alga_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://alga-agent.hahnavi.my.id/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://alga-agent.hahnavi.my.id/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  alga config edit")
    print_info("   Open config in your editor:  alga config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from alga_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Alga from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'alga setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     alga config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_alga_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo alga gateway install --system")
                print_info("  Or run in foreground:  alga gateway")
        else:
            from alga_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   alga gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   alga gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `alga setup tools` and `alga tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``alga_cli.auth`` — lists every supported
    ...
    """
    try:
        from alga_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from alga_cli.config
    ...
    if section_key == "model":
        ...
        from alga_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_alga migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_alga", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Alga.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Alga to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Alga at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Alga at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Alga at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Alga at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Alga equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Alga config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Alga.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Alga's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(alga_home: Path) -> bool:
    ...
    print_info("Alga can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: alga claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: alga claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Alga configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Alga config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Alga (use alga claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``alga setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``alga setup --portal``)
    that gets a brand-new user from zero to a fully working Alga session
    with web/image/tts/browser tools all routed via their Portal sub.
    """
    from types import SimpleNamespace

    from alga_cli.auth_commands import auth_add_command
    from alga_cli.config import save_config
    from alga_cli.auth import get_nous_auth_status
    ...
    print(color("│     🍀 Alga Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `alga setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `alga auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `alga auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `alga model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `alga portal status` to inspect routing.")
    print_info("  Run `alga` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      alga setup           — full or quick (auto-detected)
      alga setup model     — just model/provider
      alga setup tts       — just text-to-speech
      alga setup terminal  — just terminal backend
      alga setup gateway   — just messaging platforms
      alga setup tools     — just tool configuration
      alga setup agent     — just agent settings
    """
    from alga_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🍀 Alga Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             🍀 Alga Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Alga Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, alga_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Alga configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'alga setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(alga_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Alga?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, alga_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {alga_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'alga config edit'")
    ...
    # Tune later with `alga setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, alga_home)


def _run_first_time_quick_setup(config: dict, alga_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``alga setup <section>``
    or switch providers with ``alga model``.
    """
    ...
    print()
    try:
        from alga_cli.main import _model_flow_nous
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'alga setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    alga setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  alga setup gateway")
    print()

    _print_setup_summary(config, alga_home)


def _run_quick_setup(config: dict, alga_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'alga setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Alga to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'alga setup gateway'.")
