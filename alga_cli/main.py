#!/usr/bin/env python3
"""
Alga CLI - Main entry point.

Usage:
    alga                     # Interactive chat (default)
    alga chat                # Interactive chat
    alga gateway             # Run gateway in foreground
    alga gateway start       # Start gateway as service
    alga gateway stop        # Stop gateway service
    alga gateway status      # Show gateway status
    alga gateway install     # Install gateway service
    alga gateway uninstall   # Uninstall gateway service
    alga setup               # Interactive setup wizard
    alga logout              # Clear stored authentication
    alga status              # Show status of all components
    alga cron                # Manage cron jobs
    alga cron list           # List cron jobs
    alga cron status         # Check if cron scheduler is running
    alga doctor              # Check configuration and dependencies
    alga honcho setup                    # Configure Honcho AI memory integration
    alga honcho status                   # Show Honcho config and connection status
    alga honcho sessions                 # List directory → session name mappings
    alga honcho map <name>               # Map current directory to a session name
    alga honcho peer                     # Show peer names and dialectic settings
    alga honcho peer --user NAME         # Set user peer name
    alga honcho peer --ai NAME           # Set AI peer name
    alga honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    alga honcho mode                     # Show current memory mode
    alga honcho mode [hybrid|honcho|local]  # Set memory mode
    alga honcho tokens                   # Show token budget settings
    alga honcho tokens --context N       # Set session.context() token cap
    alga honcho tokens --dialectic N     # Set dialectic result char cap
    alga honcho identity                 # Show AI peer identity representation
    alga honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    alga honcho migrate                  # Step-by-step migration guide: OpenClaw native → Alga + Honcho
    alga version             Show version
    alga update              Update to latest version
    alga uninstall           Uninstall Alga Agent
    alga acp                 Run as an ACP server for editor integration
    alga sessions browse     Interactive session picker with search

    alga claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: alga_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``alga_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``alga update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``alga_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, alga crashes on import and the user can't run
# ``alga update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import alga_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'alga' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``alga tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``alga.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("alga")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"alga", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"alga")
        # Windows: the .exe name is already ``alga.exe`` — nothing to do.
    except Exception:
        pass
...
# `entry.tsx`; this is just the earlier cousin. ``ALGA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ALGA_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("ALGA_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`alga --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from alga_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Alga Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``alga --version`` before config/logging imports on Termux."""
    if os.environ.get("ALGA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ALGA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (alga tools, alga setup, alga model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'alga {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any alga module import.
#
# Many modules cache ALGA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ALGA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.alga/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ALGA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors alga_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ALGA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.alga/profiles/coder or
    # /opt/data/profiles/coder).  If ALGA_HOME points to the alga root
    # instead (e.g. systemd hardcodes ALGA_HOME=/root/.alga), we must
    # still read active_profile — the user may have switched profiles via
    # `alga profile use` and the gateway should honour that choice.
    # See issue #22502.
    alga_home_env = os.environ.get("ALGA_HOME", "")
    if profile_name is None and alga_home_env:
        if Path(alga_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the alga root
    if profile_name is None:
        try:
            from alga_constants import get_default_alga_root

            active_path = get_default_alga_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ALGA_HOME
    if profile_name is not None:
        try:
            from alga_cli.profiles import resolve_profile_env

            alga_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent alga from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ALGA_HOME"] = alga_home
...
# Load .env from ~/.alga/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from alga_cli.config import get_alga_home
from alga_cli.env_loader import load_alga_dotenv

load_alga_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ALGA_REDACT_SECRETS env
# var BEFORE alga_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_alga_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "ALGA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ALGA_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `alga` subcommands
...
try:
    from alga_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from alga_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if alga_constants not importable yet
...
from alga_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_alga_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ALGA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ALGA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from alga_cli.config import get_env_path, get_alga_home, load_config
    from alga_cli.auth import get_auth_status

    # Determine whether Alga itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from alga_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_alga_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from alga_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_alga_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Alga has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Alga to use their tokens.
    if _has_alga_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from alga_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, alga_bin
        cli_args: the original CLI arguments (everything after 'alga')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    alga_bin = container_info["alga_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo alga {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo alga {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  alga --tui --resume {target}")
    if title:
        print(f'  alga --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @alga/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@alga" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/alga-ink/package.json",
    "packages/alga-ink/package-lock.json",
    "packages/alga-ink/index.js",
    "packages/alga-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ALGA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ALGA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ALGA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ALGA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    alga_home = os.environ.get("ALGA_HOME") or str(Path.home() / ".alga")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ALGA_HOME": alga_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(alga_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(alga_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if alga_cli_dir is None:
        alga_cli_dir = Path(__file__).parent
    bundled = alga_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ALGA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ALGA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from alga_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ALGA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ALGA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ALGA_TUI_DIR (e.g. `unset ALGA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ALGA_QUIET"):
    ...
    if tui_dev:
        # Keep the local @alga/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @alga/ink resolves through
        # packages/alga-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "alga-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from alga_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="alga-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ALGA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ALGA_PYTHON_SRC_ROOT"] = os.environ.get(
        "ALGA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ALGA_PYTHON", sys.executable)
    env.setdefault("ALGA_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ALGA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ALGA_MODEL"] = model
        env["ALGA_INFERENCE_MODEL"] = model
    if provider:
        env["ALGA_TUI_PROVIDER"] = provider
        env["ALGA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ALGA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ALGA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ALGA_TUI_SKILLS"] = value
    if query:
        env["ALGA_TUI_QUERY"] = query
    if image:
        env["ALGA_TUI_IMAGE"] = image
    if checkpoints:
        env["ALGA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ALGA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ALGA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ALGA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ALGA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ALGA_ACCEPT_HOOKS"] = "1"
    ...
    # ALGA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `alga --tui` try to
    ...
    # still set ALGA_TUI_RESUME themselves.
    env.pop("ALGA_TUI_RESUME", None)
    if resume_session_id:
        env["ALGA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `alga update` so
    ...
    if code == 42:
        from alga_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ALGA_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``alga kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``alga kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ALGA_KANBAN_BOARD"):
        return
    try:
        from alga_cli.kanban_db import get_current_board

        os.environ["ALGA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.alga/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Alga — chat, and gateway. The skills_sync module is manifest-based and idempotent:
    ...
    dependency. Alga still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ALGA_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'alga sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from alga_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from alga_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'alga doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Alga isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  alga setup")
        print()

        from alga_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'alga setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from alga_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ALGA_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.alga/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ALGA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ALGA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ALGA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from alga_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from alga_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from alga_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Alga?")
        print()
    ...
    # has no creds.json.  Every subsequent `alga gateway` then paid a 30s
    ...
    session_dir = get_alga_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: alga gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `alga gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  alga gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '🍀 Alga Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  alga gateway")
        ...
        print()
        print("  Or install as a service: alga gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'alga whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from alga_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from alga_cli.config import stamp_install_method
    from alga_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("🍀 Alga post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from alga_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``alga model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from alga_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from alga_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from alga_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ALGA_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from alga_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'alga model' for "
                "available providers, or run 'alga doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in alga_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.alga/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.alga/.env if the active provider is not 'custom'.
    ...
    """
    from alga_cli.config import get_env_value, save_env_value, load_config
...
#
# Alga uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `alga model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `alga model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`alga_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from alga_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from alga_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from alga_cli.config import load_config
    ...
    while True:
        ...
        print("  Alga only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``alga model`` flow, then route aux tasks to them here.
    """
    from alga_cli.config import load_config
    from alga_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from alga_cli.auth import _prompt_model_selection
    from alga_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from alga_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from alga_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from alga_constants import OPENROUTER_BASE_URL
    from alga_cli.auth import (
        ProviderConfig,
    ...
    from alga_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.alga/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `alga setup` from scratch. OpenRouter
    ...
    from alga_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from alga_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from alga_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from alga_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from alga_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `alga auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from alga_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from alga_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``alga model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``alga model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``alga auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from alga_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from alga_cli.auth import _save_model_choice, deactivate_provider
    from alga_cli.config import get_env_value, load_config, save_config
    from alga_cli.secret_prompt import masked_secret_prompt
    ...
    from alga_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `alga model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from alga_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Alga URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from alga_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from alga_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from alga_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Alga will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Alga will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        from alga_cli.curses_ui import curses_radiolist
...
# downstream call sites read `alga_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `alga` invocation — including
# fast paths like `alga --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from alga_cli.config import save_env_value, load_config, save_config
    from alga_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from alga_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from alga_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from alga_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Alga turns to `copilot --acp`.")
    print("  Alga currently starts its own ACP subprocess for each request.")
    print("  Alga uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ALGA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``alga setup`` / ``alga model``.
    ...
    recover from a malformed paste without editing ``~/.alga/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from alga_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from alga_cli.config import save_env_value
    from alga_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `alga setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from alga_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so alga knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Alga typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Alga, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from alga_cli.auth import AuthError
        from alga_cli.models import fetch_lmstudio_models
        ...
        from alga_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from alga_constants import display_alga_home as _dhh_fn

            print(
                f"    Alga will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from alga_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'alga model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Alga CLI with a provider."""
    from alga_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from alga_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from alga_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from alga_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from alga_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from alga_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``alga slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: alga slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `alga slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from alga_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from alga_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from alga_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from alga_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `alga security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from alga_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from alga_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from alga_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from alga_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Alga home directory to a zip file."""
    if getattr(args, "quick", False):
        from alga_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from alga_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Alga backup from a zip file."""
    from alga_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from alga_cli.banner import check_for_updates
        from alga_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Alga Agent."""
    _require_tty("uninstall")
    from alga_cli.uninstall import run_uninstall
...
# Critical files that every ``alga`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``alga update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "alga_cli/main.py",
    "alga_cli/config.py",
    "alga_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``alga`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="alga-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``alga update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from alga_constants import get_alga_home
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``alga update`` to stash the
    lockfile — repeatedly.
    """
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `alga update`.
    ...
    print("  Preview now:  alga curator run --dry-run")
    print("  Pause it:     alga curator pause")
    print(
        "  Docs:         https://alga-agent.hahnavi.my.id/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``alga update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``alga update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: alga curator status)"
    )
...
def _update_via_zip(args):
    """Update Alga Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `alga update "
            f"--branch {branch}`, or update against main with `alga update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/hahnavi/alga-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="alga-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"alga-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for alga-agent itself
        ...
        # GitHub ZIPs extract to alga-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"alga-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "alga-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Alga behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes alga completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Alga couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Alga couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Alga behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `alga update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/hahnavi/alga-agent.git",
    "git@github.com:hahnavi/alga-agent.git",
    "https://github.com/hahnavi/alga-agent",
    "git@github.com:hahnavi/alga-agent",
}
OFFICIAL_REPO_URL = "https://github.com/hahnavi/alga-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_alga_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_alga_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Alga repository.")
        print("  This means you may miss updates from hahnavi/alga-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/hahnavi/alga-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/hahnavi/alga-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``alga update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from alga_constants import get_default_alga_root

    default_home = get_default_alga_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``alga update`` is still progressing even if pip/uv itself is silent.
    """
...
def _alga_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "alga.exe",
        scripts_dir / "alga-gateway.exe",
    ]


def _detect_concurrent_alga_instances(
    ...
    The Alga Desktop Electron app spawns ``alga.EXE`` as a backend child,
    so during ``alga update`` the user-invoked process and the desktop's
    ...
    shims (``alga.exe`` / ``alga-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``alga update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``alga.exe``) is a separate process from the Python
    ...
    for shim in _alga_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated alga.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``alga update``
    ...
    #      alga.exe sitting *under* a non-Alga parent (e.g. a Alga
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "alga.exe"
    lines = ["✗ Another alga.exe is running:"]
    ...
    lines.append("")
    lines.append("  Exit any open `alga` REPLs, and")
    lines.append("  stop the gateway (`alga gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `alga update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_alga_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``alga.exe``.
    ...
    when ``alga update`` runs, ``alga.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``alga.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next alga invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Alga Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Alga Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _alga_exe_shims(scripts_dir):
        ...
        print(
            "    Close Alga Desktop, exit other `alga` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `alga update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_alga_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``alga.exe.old.*`` left by prior updates.

    Called early on every alga invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``alga update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `alga update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``alga.exe`` / ``alga-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_alga_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_alga_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from alga_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.alga/logs/update.log`` so nothing is lost.
        #
...
class _UpdateOutputStream:
    """Stream wrapper used during ``alga update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.alga/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``alga update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``alga update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.alga/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``alga update --gateway``) the update is already
    ...
    try:
        ...
        # alga_cli.config.get_alga_home to simulate setup failure.
        from alga_cli.config import get_alga_home as _get_alga_home

        logs_dir = _get_alga_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``alga update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from alga_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``alga update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from alga_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from alga_cli.config import recommended_update_command
        from alga_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``alga update`` without requiring a reinstall.  The
    ...
    ``alga`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/alga.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v alga'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/alga, code at /usr/local/lib/alga-agent).
    fhs_link = Path("/usr/local/bin/alga")
    ...
    path_comment = (
        "# Alga Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ALGA_HOME before running the update.
    ...
    ALGA_HOME directories.  The ``--backup`` flag on ``alga update``
    ...
    try:
        from alga_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_alga_home so the user sees ~/.alga/...
    try:
        from alga_constants import get_alga_home, display_alga_home

        home = get_alga_home()
        try:
            display_path = f"{display_alga_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  alga import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``alga update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Alga Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Alga Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `alga update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Alga via pip (for PyPI installs)."""
    from alga_cli import __version__
    from alga_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "alga-agent"]
        ...
        cmd = [pipx, "upgrade", "alga-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "alga-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "alga-agent"]
    ...
    print("✓ Update complete! Restart alga to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("🍀 Updating Alga Agent...")
    print()

    # On Windows, abort early if another alga.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_alga_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ALGA_HOME, this is cheap
        ...
        try:
            from alga_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in alga_cli/config.py bricked
        # every user who ran ``alga update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``alga
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``alga update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_alga_home added to alga_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload alga_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_alga_home() added since the last release.
        try:
            import importlib
            import alga_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ALGA_HOME so
        # it is not affected by sync_skills()'s module-level ALGA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ALGA_HOME env var points at the default or a named profile.
        try:
            from alga_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'alga config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: alga config migrate")
            else:
                print()
                print("Skipped. Run 'alga config migrate' later to configure.")
        else:
        ...
        try:
            from alga_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `alga curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``alga update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from alga_cli.tools_config import install_cua_driver
        ...
        # When running as ``alga update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_alga_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all alga-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. alga-gateway.service or alga-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: alga gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: alga -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Alga gateway unit files are still installed.
        # When both alga.service (from a pre-rename install) and the
        # current alga-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `alga update` surfaces the issue until the user migrates.
        try:
            from alga_cli.gateway import (
                has_legacy_alga_units,
                _find_legacy_alga_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_alga_units():
                print()
                print("⚠ Legacy Alga gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_alga_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (alga.service) fight the current")
                print("  alga-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    alga gateway migrate-legacy")
                print()
        ...
        print("  alga model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``alga -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from alga_constants import display_alga_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `alga profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_alga_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → alga -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.alga)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  alga profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     alga -p {name} chat")
                else:
        ...
        from alga_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from alga_constants import get_alga_home as _hh
        ...
        from alga_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `alga profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from alga_cli.profiles import profile_exists
        ...
        from alga_cli.profiles import rename_profile
        ...
        from alga_cli.profiles import export_profile
        ...
        from alga_cli.profiles import import_profile
        ...
        import tempfile
        from alga_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="alga_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  alga -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      alga -p {plan.manifest.name} chat")
        ...
        from alga_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `alga profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"alga -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from alga_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("alga_requires"):
            print(f"Requires:     Alga {data['alga_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from alga_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.alga_requires:
        print(f"  Requires: Alga {mf.alga_requires}")
...
def cmd_completion(args, parser=None):
    ...
    from alga_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from alga_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Alga log files."""
    from alga_cli.logs import tail_log, list_logs
...
# so that in ``alga -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``alga_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``alga -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``alga`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ALGA_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from alga_cli.plugins import discover_plugins
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from alga_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("ALGA_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from alga_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from alga_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ALGA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ALGA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ALGA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `alga --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("ALGA_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("ALGA_TUI") == "1"):
        return False
...
def main():
    """Main entry point for alga CLI."""
    # Cosmetic: make the process show up as 'alga' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from alga_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``alga.exe.old.*`` quarantine files left by previous
    # ``alga update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_alga_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: alga-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from alga_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://alga-agent.hahnavi.my.id/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `alga model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.alga/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://alga-agent.hahnavi.my.id/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from alga_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from alga_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy alga.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Alga gateway unit files "
            "(e.g. alga.service) left over from older installs. Profile "
            "units (alga-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `alga proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Alga Agent with an interactive wizard. "
        "Run a specific section: alga setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'alga setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Alga.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ALGA_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Alga")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from alga_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Alga CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: alga-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Alga with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ALGA_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Alga Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.alga/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Alga profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.alga/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Alga profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from alga_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from alga_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.alga/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.alga/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Alga Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `alga "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Alga "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.alga/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Alga Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Alga setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Alga Agent. Use 'alga debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    alga debug share              Upload debug report and print URL
    alga debug share --lines 500  Include more log lines
    alga debug share --expire 30  Keep paste for 30 days
    alga debug share --local      Print report locally (no upload)
    alga debug share --no-redact  Disable upload-time secret redaction
    alga debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'alga debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Alga home directory to a zip file",
        description="Create a zip archive of your entire Alga configuration, "
        "skills, sessions, and data (excludes the alga-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/alga-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.alga/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo alga uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from alga_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Alga backup from a zip file",
        description="Extract a previously created Alga backup into your "
        "Alga home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Alga Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from alga_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.alga/skills/.bundled_manifest) "
            "so future 'alga update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from alga_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from alga_cli.skills_hub import skills_command
    ...
    from alga_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/alga-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `alga plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from alga_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``alga --help``, ``alga version``, ``alga logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from alga_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from alga_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_alga_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_alga_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_alga_home()}/memories/\n")
        else:
            from alga_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'alga tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # alga tools disable <name...> [--platform cli]
    ...
    # alga tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from alga_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from alga_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `alga computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `alga tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: alga computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: alga computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Alga as an MCP server",
        description=(
            "Manage MCP server connections and run Alga as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'alga mcp add' to connect to a new server, or\n"
            "'alga mcp serve' to expose Alga conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Alga as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `alga mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `alga mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `alga mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from alga_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_alga_home() / "sessions"
            ...
            # Launch alga --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Alga",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Alga",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.alga/ (by default a "
        "single restore-point archive is written to ~/.alga/backups/ "
        "before apply; restorable with 'alga import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from alga_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Alga Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'alga config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, alga will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another alga.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Alga Agent",
        description="Remove Alga Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Alga Agent as an ACP (Agent Client Protocol) server",
        description="Start Alga Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Alga ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Alga provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.alga/node/ "
    ...
    def cmd_acp(args):
        """Launch Alga Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Alga instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `alga update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `alga profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Alga profile distribution. SOURCE can be a git URL "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Alga log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    alga logs                    Show last 50 lines of agent.log
    alga logs -f                 Follow agent.log in real time
    alga logs errors             Show last 50 lines of errors.log
    alga logs gateway -n 100     Show last 100 lines of gateway.log
    alga logs gui -f             Follow gui.log in real time
    alga logs --level WARNING    Only show WARNING and above
    alga logs --session abc123   Filter by session ID
    alga logs --component tools  Only show tool-related lines
    alga logs --since 1h         Lines from the last hour
    alga logs --since 30m -f     Follow, starting from 30 min ago
    alga logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``alga -c Pokemon Agent Dev`` → ``alga -c 'Pokemon Agent Dev'``
    ...
    from alga_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'alga -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (alga hooks list, cron
