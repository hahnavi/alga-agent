"""Profile distributions — shareable, packaged Alga profiles via git.

A distribution is a Alga profile published as a git repository (or
...
* ``alga profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``alga skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``alga profile``, not a parallel tree):

    alga profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "alga-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            alga_requires=str(data.get("alga_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.alga_requires:
            out["alga_requires"] = self.alga_requires
        if self.author:
...
def check_alga_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Alga {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``alga profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Alga profile distribution."
            )
...
def plan_install(
    ...
    from alga_cli import __version__ as alga_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Alga distribution."
        )

    # Version check up-front so we fail fast
    check_alga_requires(manifest.alga_requires, alga_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.alga).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="alga_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `alga profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`alga profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="alga_dist_update_") as tmp:
        plan = plan_install(
