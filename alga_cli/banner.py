...
Pure display functions with no AlgaCLI state dependency.
"""
...
from alga_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built alga — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/hahnavi/alga-agent.git"
...
def _fetch_pypi_latest(package: str = "alga-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Alga update is available.

    Two paths: if ``ALGA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = alga_home / ".update_check"
    embedded_rev = os.environ.get("ALGA_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $ALGA_HOME/alga-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = alga_home / "alga-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Alga git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ALGA_HOME/alga-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``alga_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from alga_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/hahnavi/alga-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Alga checkout. Cached per-process. Release URL always points at the
    canonical hahnavi/alga-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Alga Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ALGA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ALGA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from alga_cli.codex_runtime_switch import get_current_runtime
        from alga_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from alga_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built alga; we know an update
    ...
    # Pip-install warning — `pip install alga-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ALGA_AGENT_LOGO
        console.print(_logo)
        console.print()
