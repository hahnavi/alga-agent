"""
Alga Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.alga/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Alga PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing alga-agent or alga PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Alga Agent" comment and following line
                if '# Alga Agent' in line or '# alga-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('alga' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing alga
                if 'alga' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the alga wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "alga",
        Path("/usr/local/bin/alga"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains alga_cli reference)
                content = wrapper.read_text()
                if 'alga_cli' in content or 'alga-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(alga_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $ALGA_HOME/node/bin/node
        ~/.local/bin/npm  -> $ALGA_HOME/node/bin/npm
        ~/.local/bin/npx  -> $ALGA_HOME/node/bin/npx
    ...
    when the link still resolves into this Alga home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Alga) is left untouched so we never break unrelated tooling.
    """
    node_dir = (alga_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``alga gateway run`` processes
    ...
    try:
        from alga_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from alga_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ALGA_HOME`` and ``ALGA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\alga\git\cmd``, ``%LOCALAPPDATA%\alga\git\bin``,
#      ``%LOCALAPPDATA%\alga\git\usr\bin``, ``%LOCALAPPDATA%\alga\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\alga\git\`` and Node to
#      ``%LOCALAPPDATA%\alga\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``alga dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\alga\gateway-service\`` and sometimes
...
def _alga_path_markers(alga_home: Path) -> list[str]:
    """Path-entry substrings that identify Alga-owned User-PATH entries."""
    root = str(alga_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare alga-agent install dir.
    markers = [root + "\\alga-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ALGA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "alga".  We don't want to catch
    # unrelated entries like "calga-foo" or "ephermeral", so we look for
    # backslash-alga as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(alga_home: Path) -> list[str]:
    """Strip Alga-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _alga_path_markers(alga_home)
            kept: list[str] = []
...
def remove_alga_env_vars_windows() -> list[str]:
    """Delete ALGA_HOME and ALGA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ALGA_HOME", "ALGA_GIT_BASH_PATH"):
                try:
...
def _is_default_alga_home(alga_home: Path) -> bool:
    """Return True when ``alga_home`` points at the default (non-profile) root."""
    try:
        from alga_constants import get_default_alga_root
        return alga_home.resolve() == get_default_alga_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ALGA_HOME directory.

    We shell out to ``alga -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ALGA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m alga_cli.main` so we don't depend on a `alga`
    #    wrapper that may be half-removed mid-uninstall.
    alga_invocation = [_sys.executable, "-m", "alga_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                alga_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ALGA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.alga/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.alga/ for future reinstall
    """
    project_root = get_project_root()
    alga_home = get_alga_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ALGA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_alga_home(alga_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            🍀 Alga Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {alga_home / 'config.yaml'}")
    print(f"  Secrets: {alga_home / '.env'}")
    print(f"  Data:    {alga_home / 'cron/'}, {alga_home / 'sessions/'}, {alga_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ALGA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Alga data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Alga code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in alga_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\alga\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(alga_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Alga-owned PATH entries in User environment")

        log_info("Removing ALGA_HOME / ALGA_GIT_BASH_PATH User env vars...")
        removed_env = remove_alga_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Alga-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing alga command...")
    ...
    #     (only when they still point into this Alga home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Alga-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(alga_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Alga-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.alga/, just remove the alga-agent subdir
            if alga_home in project_root.parents or project_root.parent == alga_home:
    ...
    #     under ALGA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(alga_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(alga_home)
    ...
    # 5. Optionally remove ~/.alga/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ALGA_HOME dirs live under
        ...
        try:
            if alga_home.exists():
                shutil.rmtree(alga_home)
                log_success(f"Removed {alga_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {alga_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {alga_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/hahnavi/alga-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/hahnavi/alga-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Alga Agent! 🍀")
    print()
