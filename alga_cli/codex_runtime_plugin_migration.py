"""Migrate Alga' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Alga' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Alga mcp_servers.<n>.command/args/env  → codex stdio transport
  Alga mcp_servers.<n>.url/headers       → codex streamable_http transport
  Alga mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Alga mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Alga-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Alga' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Alga config.")
...
# Alga keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Alga' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, alga_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Alga MCP server config to the codex inline-table dict
    ...
    if not isinstance(alga_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(alga_cfg.get("command"))
    has_url = bool(alga_cfg.get("url"))
    ...
    for key in alga_cfg:
        ...
        elif key not in _KNOWN_ALGA_KEYS:
            skipped.append(f"{key} (unknown Alga key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ALGA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Alga)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Alga' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Alga' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Alga
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="alga-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ALGA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed alga-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.alga`` symlink that happens to live under ``/private/var/folders``
...
def _build_alga_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Alga' own
    ...
    so a alga installed under /opt/, /usr/local/, or a venv all work.
    ALGA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ALGA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_alga_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ALGA_HOME its launcher sets
    ...
    # — that would override the launcher's ALGA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ALGA_HOME", tmp_path) would
    ...
    alga_home = os.environ.get("ALGA_HOME") or ""
    if alga_home and _looks_like_test_tempdir(alga_home):
        alga_home = ""
    if alga_home:
        env["ALGA_HOME"] = alga_home
    # PYTHONPATH passes through so a worktree-launched alga finds the
    ...
    env["ALGA_QUIET"] = "1"
    env["ALGA_REDACT_SECRETS"] = env.get("ALGA_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_alga_tools: bool = True,
) -> MigrationReport:
    """Translate Alga mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        alga_config: full ~/.alga/config.yaml dict
        ...
        expose_alga_tools: when True (default), register Alga' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Alga for tools
    ...
    alga_servers = (alga_config or {}).get("mcp_servers") or {}
    if not isinstance(alga_servers, dict):
        report.errors.append(
            "mcp_servers in Alga config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in alga_servers.items():
    ...
    # Inject Alga' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Alga for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/alga_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_alga_tools:
        translated["alga-tools"] = _build_alga_tools_mcp_entry()
        if "alga-tools" not in report.migrated:
            report.migrated.append("alga-tools")
