"""
alga fallback — manage the fallback provider chain.
...
Subcommands:
  alga fallback [list]   Show the current fallback chain (default when no subcommand)
  alga fallback add      Pick provider + model via the same picker as `alga model`,
                           then append the selection to the chain
  alga fallback remove   Pick an entry to delete from the chain
  alga fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.alga/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from alga_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `alga model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  alga fallback add")
        print()
        return
    ...
    print("  Docs: https://alga-agent.hahnavi.my.id/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `alga model`, then append the selection to the chain."""
    from alga_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `alga model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `alga fallback list` to view, or `alga fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``alga fallback [subcommand]``."""
