"""
Single source of truth for provider identity in Alga Agent.
...
2. **Alga overlays** — transport type, auth patterns, aggregator flags,
...
# -- Alga overlay ----------------------------------------------------------
# Alga-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AlgaOverlay:
    """Alga-specific provider metadata layered on top of models.dev."""
...
ALGA_OVERLAYS: Dict[str, AlgaOverlay] = {
    "openrouter": AlgaOverlay(
    ...
    "openai-codex": AlgaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": AlgaOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": AlgaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AlgaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ALGA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AlgaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": AlgaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AlgaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AlgaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AlgaOverlay(
    ...
    "minimax-oauth": AlgaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AlgaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AlgaOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": AlgaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": AlgaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AlgaOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "alga", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Alga overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Alga overlay
    ...
    overlay = ALGA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + alga extra
    ...
    if overlay is not None:
        # Alga-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ALGA_OVERLAYS
    if provider == "bedrock":
