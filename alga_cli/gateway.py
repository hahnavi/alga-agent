"""
Gateway subcommand for alga CLI.

Handles: alga gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_alga_home is imported lazily at call sites to avoid ImportError
# when alga_constants is cached from a pre-update version during `alga update`.
...
from alga_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "alga CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``alga gateway status`` from falsely counting the ``alga`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``alga gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "alga_cli.main gateway",
        "alga_cli.main --profile",
        "alga_cli.main -p",
        "alga_cli/main.py gateway",
        "alga_cli/main.py --profile",
        "alga_cli/main.py -p",
        "alga gateway",
        ...
        # ``alga.exe --profile`` / ``alga.exe -p`` would also match
        # ``alga.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "alga.exe gateway",
        "alga-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_alga_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"alga_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ALGA_HOME
        ...
        # ALGA_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "alga_home=" in command_lc
            and f"alga_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``alga update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Alga profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Alga profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from alga_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "alga_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``alga update`` users
    # with no running gateway until they re-invoke ``alga gateway``
...
def _sync_alga_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ALGA_HOME``.

    Under ``sudo``, ``ALGA_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_alga_home` falls back to ``/root/.alga`` — the wrong
    profile. The unit file pins ``ALGA_HOME`` for the actual gateway
    ...
    unit_home = env.get("ALGA_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ALGA_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ALGA_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}alga gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}alga gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from alga_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from alga_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `alga gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``alga gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from alga_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ALGA_HOME-scoped).
...
from alga_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Alga Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``alga gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ALGA_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ALGA_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "alga-gateway"
SERVICE_DESCRIPTION = "Alga Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ALGA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ALGA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from alga_constants import get_default_alga_root

    home = get_alga_home().resolve()
    default = get_default_alga_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ALGA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(alga_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ALGA_HOME is a named profile.

    For ``~/.alga/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        alga_home: Optional explicit ALGA_HOME path. Defaults to the current
            ``get_alga_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from alga_constants import get_default_alga_root

    home = Path(alga_home or str(get_alga_home())).resolve()
    default = get_default_alga_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ALGA_HOME.

    Default ``~/.alga`` returns ``alga-gateway`` (backward compatible).
    Profile ``~/.alga/profiles/coder`` returns ``alga-gateway-coder``.
    Any other ALGA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Alga installs that predate the
# alga-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (alga-gateway-*.service) and unrelated third-party
# "alga" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("alga.service",)
...
def _find_legacy_alga_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Alga gateway units.

    Detects unit files installed by older Alga versions that used a
    different service name (e.g. ``alga.service`` before the rename to
    ``alga-gateway.service``). When both a legacy unit and the current
    ``alga-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``alga-gateway-coder.service`` and unrelated third-party
      ``alga-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``alga.service`` running an unrelated
...
def has_legacy_alga_units() -> bool:
    """Return True when any legacy Alga gateway unit files exist."""
    return bool(_find_legacy_alga_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Alga gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_alga_units()
    if not legacy:
        return
    print_warning("Legacy Alga gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current alga-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    alga gateway migrate-legacy")


def remove_legacy_alga_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Alga gateway unit files.

    Iterates over whatever ``_find_legacy_alga_units()`` returns — which is
    ...
    legacy = _find_legacy_alga_units()
    if not legacy:
        print("No legacy Alga gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Alga gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: alga gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo alga gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    alga gateway uninstall")
    print_info("    sudo alga gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Alga can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo alga gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo alga gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo alga gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Alga often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.alga`` → ``ai.alga.gateway.plist`` (backward compatible).
    Profile ``~/.alga/profiles/coder`` → ``ai.alga.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.alga.gateway-{suffix}" if suffix else "ai.alga.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.alga/alga-agent  -> /home/alice/.alga/alga-agent
      /opt/alga                 -> /opt/alga  (kept as-is)
...
def _alga_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ALGA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_alga_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.alga                    → /home/alice/.alga
      /root/.alga/profiles/coder     → /home/alice/.alga/profiles/coder
      /opt/custom-alga               → /opt/custom-alga  (kept as-is)
    """
    current_alga = get_alga_home().resolve()
    current_default = (Path.home() / ".alga").resolve()
    target_default = Path(target_home_dir) / ".alga"

    # Default ~/.alga → remap to target user's default
    if current_alga == current_default:
        return str(target_default)

    # Profile or subdir of ~/.alga → preserve the relative structure
    try:
        relative = current_alga.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.alga) — keep as-is
        return str(current_alga)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    alga_home = get_alga_home()
    alga_node = alga_home / "node" / "bin"
    if _is_dir(alga_node):
        candidates.append(str(alga_node))
    alga_nm = alga_home / "node_modules" / ".bin"
    if _is_dir(alga_nm):
        candidates.append(str(alga_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m alga_cli.main``, so module
    ...
    dir, or a clone that ``alga update`` later relocates/removes — the path
    ...
    ``ALGA_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ALGA_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        alga_home = _alga_home_for_target_user(home_dir)
        profile_arg = _profile_arg(alga_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ALGA_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(alga_home) if alga_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m alga_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ALGA_HOME={alga_home}"
Restart=always
RestartSec=5
...
"""
    ...
    alga_home = str(get_alga_home().resolve())
    profile_arg = _profile_arg(alga_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ALGA_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ALGA_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../alga_test`` path into
    # ``Environment="ALGA_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Alga install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo alga gateway uninstall --system")
    print_info("         alga gateway install")
    print_info("         alga gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (alga.service from pre-rename installs)
    # before installing the new alga-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}alga gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}alga gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}alga gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_alga_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `alga gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `alga gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}alga gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}alga gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}alga gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}alga gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.alga.gateway-{suffix}" if suffix else "ai.alga.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Alga install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  alga gateway status             # Check status")
    from alga_constants import display_alga_home as _dhh
...
def launchd_stop():
    ...
    # `alga gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Alga install")
    else:
        print("⚠ Service definition is stale relative to the current Alga install")
        print("  Run: alga gateway start")
    ...
    if deep:
        log_file = get_alga_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ALGA_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Alga gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'alga' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/alga/docker/entrypoint.sh before the Alga command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ALGA_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ALGA_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `alga gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ALGA_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `alga gateway restart` which already
    ...
    # `alga gateway start/restart` — leaving the gateway vulnerable to
    ...
    # ALGA_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ALGA_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Alga will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @alga:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. alga) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Alga delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Alga agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Alga will use (e.g., alga@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Alga connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: alga pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Alga will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``alga setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run alga under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.alga/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_alga_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'alga pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: alga pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'alga gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from alga_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from alga_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: alga pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'alga gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from alga_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Alga will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Alga will store the returned account_id/token in ~/.alga/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `alga gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_alga_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `alga pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `alga pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AlgaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``alga_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from alga_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.alga/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.alga/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: alga gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: alga gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: alga gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo alga gateway install --system"
                        )
                    print_info("  Or run in foreground:  alga gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: alga gateway run")
                print_info(
                    "  For persistence:   tmux new -s alga 'alga gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: alga gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup alga gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'alga gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`alga_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from alga_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ALGA_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``alga gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``alga gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``alga gateway run`` are unaffected.
      2. ``ALGA_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``alga gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ALGA_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ALGA_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ALGA_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ALGA_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  Use `--no-supervise` (or ALGA_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    # gateway flaps. `sleep infinity` matches the static main-alga
    # service's pattern (see docker/s6-rc.d/main-alga/run): the CMD
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: alga gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: alga gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s alga 'alga gateway run'"
                )
                print()
            ...
            print()
            print(
                "  alga gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s alga 'alga gateway run'         # persistent via tmux"
            )
            print(
                "  nohup alga gateway run > ~/.alga/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  alga profile create <name>     # creates the s6 service slot")
                print("  alga -p <name> gateway start   # bring it up via s6")
                print("  alga status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: alga gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: alga gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: alga gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  alga profile delete <name>     # tears down the s6 service slot")
                print("  alga -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `alga -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: alga gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ALGA_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `alga gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ALGA_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `alga gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: alga gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  alga gateway install")
                else:
                    print("To install as a service:")
                    print("  alga gateway install")
                    print("  sudo alga gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  alga gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup alga gateway run > ~/.alga/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s alga 'alga gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  alga gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  alga gateway install  # Install as user service")
                    print(
                        "  sudo alga gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Alga gateway unit files from
        # pre-rename installs (e.g. alga.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_alga_units(interactive=not yes, dry_run=dry_run)
