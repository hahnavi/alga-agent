...
Used by ``alga kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``alga_cli/goals.py`` — same aux
...
ALGA_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("ALGA_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Alga Agent board.
...
def _profile_author() -> str:
    """Mirror of ``alga_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ALGA_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=ALGA_KANBAN_SPECIFY_MAX_TOKENS,
