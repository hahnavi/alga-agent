...
Reads the user's Nous OAuth state from ``~/.alga/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `alga auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `alga auth add nous` to re-authenticate."
                )
    ...
    # to alga_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
