...
# xAI's public API is OpenAI-compatible for the endpoints Alga commonly
# uses. The Responses endpoint is included because Alga' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Alga-managed OAuth credentials."""

    auth_hint = "alga auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`alga auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`alga auth reset xai-oauth` or re-authenticate with "
                    "`alga auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `alga auth add xai-oauth --type oauth`."
            )
