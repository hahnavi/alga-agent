"""``alga portal`` — small CLI surface for Nous Portal users.
...
already in ``alga auth`` or ``alga tools``. It's a discovery + status
...
DOCS_URL = "https://alga-agent.hahnavi.my.id/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   alga auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `alga model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `alga auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `alga portal <subcommand>`."""
    ...
    print("Run `alga portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `alga portal` on the given argparse subparsers object."""
