"""Shell completion script generation for alga CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_alga_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Alga Agent bash completion
# Add to ~/.bashrc:
#   eval "$(alga completion bash)"

_alga_profiles() {{
    local profiles_dir="$HOME/.alga/profiles"
...
_alga_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_alga_profiles)" -- "$cur"))
        return
    fi
...
complete -F _alga_completion alga
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef alga
# Alga Agent zsh completion
# Add to ~/.zshrc:
...
_alga_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.alga/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.alga/profiles 2>/dev/null)}}")
    fi
...
_alga() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_alga_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'alga command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Alga Agent fish completion",
        "# Add to your config:",
        "#   alga completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __alga_profiles",
        "    echo default",
        "    if test -d $HOME/.alga/profiles",
        "        ls $HOME/.alga/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c alga -f -s p -l profile"
        " -d 'Profile name' -xa '(__alga_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__alga_profiles)' -d 'Profile name'"
                )
