...
Used by ``alga profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``alga_cli/kanban_specify.py``: lazy aux
...
from alga_cli import profiles as profiles_mod
...
_SYSTEM_PROMPT = """You are a profile-describer for the Alga Agent kanban board.
...
Rules:
  ...
  - Never write "Alga Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from alga_constants import get_alga_home  # type: ignore
            profile_dir = Path(get_alga_home())
        else:
