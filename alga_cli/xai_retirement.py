...
Pure logic: walks a Alga config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `alga doctor` and a future `alga migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Alga config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Alga config and return retirement issues.
