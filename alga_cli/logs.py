"""``alga logs`` — view and filter Alga log files.
...
Usage examples::
    ...
    alga logs                    # last 50 lines of agent.log
    alga logs -f                 # follow agent.log in real time
    alga logs errors             # last 50 lines of errors.log
    alga logs gateway -n 100    # last 100 lines of gateway.log
    alga logs gui -f            # follow gui.log (dashboard/pty/ws)
    alga logs --level WARNING    # only WARNING+ lines
    alga logs --session abc123   # filter by session ID substring
    alga logs --component tools  # only tool-related lines
    alga logs --since 1h         # lines from the last hour
    alga logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_alga_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Alga runs — try 'alga chat' first)")
        sys.exit(1)
    ...
    if component:
        from alga_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_alga_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_alga_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_alga_home()}/logs/")
        return

    print(f"Log files in {display_alga_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'alga chat' to generate logs)")
