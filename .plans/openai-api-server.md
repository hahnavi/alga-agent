# OpenAI-Compatible API Server for Alga Agent
...
SSE streaming. By exposing this endpoint, alga-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  alga-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.alga/.env`
2. Run `alga gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with alga-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns alga-agent as a model) |
...
Authorization: Bearer alga-api-key-here
...
But alga-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "alga-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured alga-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of alga-agent's core prompt
...
Once implemented, alga-agent works as a drop-in backend for:
