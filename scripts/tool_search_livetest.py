#!/usr/bin/env python3
"""Live test harness for Alga Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any alga imports.
ORIGINAL_HOME = os.environ.get("ALGA_HOME")
ORIGINAL_AUTH = Path.home() / ".alga" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.alga/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.alga/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="alga_ts_live_"))
    alga_home = home_dir / ".alga"
    alga_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, alga_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".alga" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, alga_home / ".env")
        ...
        load_alga_dotenv(alga_home=str(Path.home() / ".alga"))
    ...
    (alga_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return alga_home
...
def reset_module_state():
    """Drop cached modules so the new ALGA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "alga_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ALGA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ALGA_HOME"] = ORIGINAL_HOME
    else:
