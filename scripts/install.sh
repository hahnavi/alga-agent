...
# Python-driven tool session (e.g. Alga terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:hahnavi/alga-agent.git"
REPO_URL_HTTPS="https://github.com/hahnavi/alga-agent.git"
ALGA_HOME="${ALGA_HOME:-$HOME/.alga}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ALGA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ALGA_INSTALL_DIR"
...
#   code at /usr/local/lib/alga-agent, command at /usr/local/bin/alga,
#   data still at /root/.alga (ALGA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --alga-home PATH  Data directory (default: ~/.alga, or \$ALGA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Alga installs the code under"
            echo "  /usr/local/lib/alga-agent and links the command into"
            echo "  /usr/local/bin/alga (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ALGA_HOME"
            echo "  (default /root/.alga).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ALGA_HOME/alga-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs alga setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `alga update`
...
# Decide where the repo checkout + venv live, and where the `alga` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ALGA_HOME/alga-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ALGA_HOME/alga-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/alga-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ALGA_HOME/alga-agent — then preserve it)
#
# Always no-op when the user set --dir or $ALGA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ALGA_HOME.
    if is_termux; then
        INSTALL_DIR="$ALGA_HOME/alga-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ALGA_HOME/alga-agent/.git" ]; then
            INSTALL_DIR="$ALGA_HOME/alga-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/alga-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/alga-agent"
        ...
        # /usr/local/bin/alga wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/alga"
        log_info "  Data:    $ALGA_HOME (unchanged)"
...
get_alga_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/alga" ]; then
        echo "$link_dir/alga"
    else
        echo "alga"
    fi
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/alga-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/alga-uv-installer.$$.sh")"
...
# module compiled in. Alga' session store (alga_state.py) creates FTS5
...
_warn_no_fts5() {
    ...
    # Install proceeds — Alga degrades gracefully and disables only full-text
    # session search — but warn so it isn't a silent gap.
    log_warn "Could not obtain an FTS5-capable Python. Alga will run, but"
...
ensure_fts5() {
    ...
    _tmp_uv_dir="$(mktemp -d 2>/dev/null || echo "/tmp/alga-fresh-uv.$$")"
    mkdir -p "$_tmp_uv_dir"
...
install_node() {
    ...
    log_info "Extracting to ~/.alga/node/..."
    ...
    local installed_ver
    installed_ver=$("$ALGA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.alga/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Alga install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Alga Agent itself does not require or retain root access."
...
setup_path() {
    log_info "Setting up alga command..."

    if [ "$USE_VENV" = true ]; then
        ALGA_BIN="$INSTALL_DIR/venv/bin/alga"
    else
        ALGA_BIN="$(which alga 2>/dev/null || echo "")"
        if [ -z "$ALGA_BIN" ]; then
            log_warn "alga not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ALGA_BIN" ]; then
        log_warn "alga entry point not found at $ALGA_BIN"
    ...
    # Create a user-facing shim for the alga command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ALGA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ALGA_BIN"` self-recurse. (#21454)
...
exec "$ALGA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/alga"
    log_success "Installed alga launcher → $command_link_display_dir/alga"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "alga command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v alga' \
                >/dev/null 2>&1; then
        ...
        log_info "alga not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Alga Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Alga Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Alga Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so alga works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.alga directory structure (config at top level, code in subdir)
    mkdir -p "$ALGA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.alga/.env (top level, easy to find)
    if [ ! -f "$ALGA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ALGA_HOME/.env"
            log_success "Created ~/.alga/.env from template"
        else
            touch "$ALGA_HOME/.env"
            log_success "Created ~/.alga/.env"
        fi
    else
        log_info "~/.alga/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ALGA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.alga/config.yaml (top level, easy to find)
    if [ ! -f "$ALGA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ALGA_HOME/config.yaml"
            log_success "Created ~/.alga/config.yaml from template"
        fi
    else
        log_info "~/.alga/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ALGA_HOME/SOUL.md" ]; then
        cat > "$ALGA_HOME/SOUL.md" << 'SOUL_EOF'
# Alga Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.alga/SOUL.md (edit to customize personality)"
    fi
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Alga Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Alga will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (alga --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `alga update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'alga setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Alga to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ALGA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'alga whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ALGA_CMD="$(get_alga_command_path)"
                $ALGA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'alga whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'alga gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ALGA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ALGA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: alga gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: alga gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ALGA_CMD gateway > "$ALGA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.alga/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: alga gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: alga gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ALGA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ALGA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ALGA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}alga${NC}              Start chatting"
    echo -e "   ${GREEN}alga setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}alga config${NC}       View/edit configuration"
    echo -e "   ${GREEN}alga config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}alga gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}alga update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'alga' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'alga' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'alga' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ALGA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ALGA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ALGA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ALGA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ALGA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Alga for pip install"
    ...
    ALGA_CMD="$(command -v alga 2>/dev/null || echo "")"
    if [ -n "$ALGA_CMD" ]; then
        log_info "Running alga setup..."
        "$ALGA_CMD" setup
    else
        log_warn "alga command not found on PATH"
        log_info "Try: python -m alga_cli.main setup"
    fi
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$ALGA_HOME/.install_method"
            ;;
        *)
