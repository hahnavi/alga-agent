#!/usr/bin/env bash
# Canonical test runner for alga-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.alga/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.alga/alga-agent/venv"; do
...
if [ -f "$HOME/.alga/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.alga"
