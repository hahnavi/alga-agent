# ============================================================================
# Alga Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AlgaHome = $(if ($env:ALGA_HOME) { $env:ALGA_HOME } else { "$env:LOCALAPPDATA\alga" }),
    [string]$InstallDir = $(if ($env:ALGA_HOME) { "$env:ALGA_HOME\alga-agent" } else { "$env:LOCALAPPDATA\alga\alga-agent" }),
...
$RepoUrlSsh = "git@github.com:hahnavi/alga-agent.git"
$RepoUrlHttps = "https://github.com/hahnavi/alga-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Alga Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AlgaHome)) {
        New-Item -ItemType Directory -Force -Path $AlgaHome | Out-Null
    }
    $envFile = Join-Path $AlgaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AlgaHome "node"
...
function Install-Git {
    ...
    which Alga uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\alga\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Alga needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Alga copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\alga\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ALGA_GIT_BASH_PATH`` (User scope) so Alga can find it in a fresh
    ...
    # Download PortableGit into $AlgaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AlgaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Alga features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Alga needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ALGA_GIT_BASH_PATH`` (User env scope) so Alga can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AlgaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AlgaHome\git\usr\bin\bash.exe
    $candidates += "$AlgaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AlgaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ALGA_GIT_BASH_PATH", $candidate, "User")
            $env:ALGA_GIT_BASH_PATH = $candidate
            Write-Info "Set ALGA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Alga may not find Git Bash."
    Write-Info "If needed, set ALGA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$AlgaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$AlgaHome\node;$env:Path"
        Write-Success "Node.js $version found (Alga-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $AlgaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AlgaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\alga-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AlgaHome\node") { Remove-Item -Recurse -Force "$AlgaHome\node" }
                Move-Item $extractedDir.FullName "$AlgaHome\node"
                ...
                $version = & "$AlgaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AlgaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\alga-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running alga processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/hahnavi/alga-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/hahnavi/alga-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/hahnavi/alga-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\alga-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\alga-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `alga.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install alga-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and alga.exe broken with
    ...
    # here, not 30 seconds later when the user runs `alga`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `alga dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `alga dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `alga dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up alga command..."
    
    if ($NoVenv) {
        $algaBin = "$InstallDir"
    } else {
        $algaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so alga is globally available
    # On Windows, the alga.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$algaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$algaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $algaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ALGA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\alga instead
    # of the Unix default ~/.alga
    $currentAlgaHome = [Environment]::GetEnvironmentVariable("ALGA_HOME", "User")
    if (-not $currentAlgaHome -or $currentAlgaHome -ne $AlgaHome) {
        [Environment]::SetEnvironmentVariable("ALGA_HOME", $AlgaHome, "User")
        Write-Success "Set ALGA_HOME=$AlgaHome"
    }
    $env:ALGA_HOME = $AlgaHome
    
    # Update current session
    $env:Path = "$algaBin;$env:Path"
    
    Write-Success "alga command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.alga-bootstrap-complete which tells the Alga
    ...
    $markerPath = Join-Path $InstallDir ".alga-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.alga directory structure
    New-Item -ItemType Directory -Force -Path "$AlgaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AlgaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AlgaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.alga/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.alga/.env"
        }
    } else {
        Write-Info "~/.alga/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$AlgaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.alga/config.yaml from template"
        }
    } else {
        Write-Info "~/.alga/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Alga's prompt-injection scanner
...
Edit this to customize how Alga communicates with you.
...
"@
        ...
        Write-Success "Created ~/.alga/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.alga/"
    
    # Seed bundled skills into ~/.alga/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.alga/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.alga/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AlgaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.alga/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Alga-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'alga setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\alga-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\alga-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\alga-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.alga/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'alga setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m alga_cli.main setup
    } else {
        python -m alga_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $algaCmd = "$InstallDir\venv\Scripts\alga.exe"
    if (-not (Test-Path $algaCmd)) {
        $algaCmd = "alga"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AlgaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'alga whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: alga gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AlgaHome\logs\gateway.log"
            Start-Process -FilePath $algaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AlgaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: alga gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: alga gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AlgaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlgaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlgaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AlgaHome\alga-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   alga              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   alga setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   alga config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   alga config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   alga gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   alga update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Alga repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Alga to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/hahnavi/alga-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
