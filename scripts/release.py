...
VERSION_FILE = REPO_ROOT / "alga_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+alga-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "alga@nousresearch.com": "NousResearch",
    ...
    "alga@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ALGA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ALGA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: alga-eval community link)
    ...
    "algaagent26@gmail.com": "algaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+algaagent26@users.noreply.github.com": "algaagent26",
    ...
    "vigo@alga": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "david@loadmagic.ai": "davidcampbelldc",  # PR #26834 (web proxy_headers=False)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Alga runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ALGA_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"alga-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "alga@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/hahnavi/alga-agent",
    ...
    lines.append(f"# Alga Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Alga Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Alga Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Alga Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Alga Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Alga Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Alga Agent v{new_version} ({calver_date})' "
