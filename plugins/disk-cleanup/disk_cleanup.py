"""disk_cleanup — ephemeral file cleanup for Alga Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ALGA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ALGA_HOME and /tmp/alga-*
Never touches: ~/.alga/logs/ or any system directory.
"""
...
try:
    ...
    def get_alga_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ALGA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".alga").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ALGA_HOME/logs/``."""
    return get_alga_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ALGA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ALGA_HOME or ``/tmp/alga-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    alga_home = get_alga_home()
    try:
        path.resolve().relative_to(alga_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/alga-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("alga-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ALGA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ALGA_HOME (but leave ALGA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "alga-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(alga_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == alga_home:
                continue
            try:
                rel_parts = dirpath.relative_to(alga_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    alga_home = get_alga_home()
    try:
        rel = path.resolve().relative_to(alga_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "alga-agent",
        }:
            return None
        ...
        # Path isn't under ALGA_HOME (e.g. /tmp/alga-*) — fall through.
        pass
