...
Auto-tracks and cleans up ephemeral files created during Alga Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ALGA_HOME` and `/tmp/alga-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALGA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ALGA_HOME` or `/tmp/alga-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ALGA_HOME/disk-cleanup/` is itself excluded
- `$ALGA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
