...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/alga>
...
```bash
alga memory setup honcho   # configure Honcho directly (works on a fresh install)
alga memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
alga config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.alga/.env
```

> `alga honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `alga memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ALGA_HOME/honcho.json` | Profile-local (isolated Alga instances) |
| 2 | `~/.alga/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Alga profile: `alga` (default) or `alga_<profile>`.
...
**Deployment shapes** (`alga memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Alga to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Alga session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`alga-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`alga`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-alga-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening alga in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Alga session ID (timestamp + hex). Every `alga` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Alga profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`alga`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Alga profile: `alga` (default) or `alga_<profile>` (e.g. `alga -p coder` -> host key `alga_coder`). Older `alga.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `alga memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `alga honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `alga memory setup`) |
| `alga honcho status` | Show resolved config for active profile |
| `alga honcho enable` / `disable` | Toggle Honcho for active profile |
| `alga honcho mode <mode>` | Change recall or observation mode |
| `alga honcho peer --user <name>` | Update user peer name |
| `alga honcho peer --ai <name>` | Update AI peer name |
| `alga honcho tokens --context <N>` | Set context token budget |
| `alga honcho tokens --dialectic <N>` | Set dialectic max chars |
| `alga honcho map <name>` | Map current directory to a session name |
| `alga honcho sync` | Create host blocks for all Alga profiles |
