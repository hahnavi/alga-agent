...
Config file: `$ALGA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alga` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `alga-{identity}` → `alga-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Alga profile:

```json
{
  "container_tag": "alga-{identity}"
}
```

For a profile named `coder`, this resolves to `alga-coder`. The default profile resolves to `alga-default`. Without `{identity}`, all profiles share the same container.
