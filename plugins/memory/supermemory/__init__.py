...
def _load_supermemory_config(alga_home: str) -> dict:
    config = _default_config()
    config_path = Path(alga_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, alga_home: str) -> None:
    config_path = Path(alga_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `alga memory setup`.
        # All other options are documented for $ALGA_HOME/supermemory.json
    ...
    def save_config(self, values, alga_home):
        ...
        _save_supermemory_config(sanitized, alga_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from alga_constants import get_alga_home
        self._alga_home = kwargs.get("alga_home") or str(get_alga_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._alga_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "alga", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "alga_memory", "target": target, "type": "explicit_memory"},
