...
> **Note:** The TUI-in-the-dashboard integration (`hermes dashboard` → `/chat` with the embedded `hermes --tui` over a PTY/WebSocket bridge) was removed when the web UI dashboard was deleted. The standalone TUI (`hermes --tui`) is unaffected and remains the primary chat experience.
