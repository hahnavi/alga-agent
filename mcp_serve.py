"""
Alga MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Alga-specific extra)

Usage:
    alga mcp serve
    alga mcp serve --verbose
...
logger = logging.getLogger("alga.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ALGA_HOME."""
    try:
        from alga_constants import get_alga_home
        return get_alga_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ALGA_HOME", Path.home() / ".alga")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from alga_constants import get_alga_home
        directory_file = get_alga_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ALGA_HOME", Path.home() / ".alga")
...
class EventBridge:
    ...
    This is the Alga equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from alga_constants import get_alga_home
            db_file = get_alga_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ALGA_HOME", Path.home() / ".alga")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Alga MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "alga",
        instructions=(
            "Alga Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Alga MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
