#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Alga Agent.
...
# Ensure sibling modules (_alga_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _alga_home import display_alga_home, get_alga_home

ALGA_HOME = get_alga_home()
TOKEN_PATH = ALGA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ALGA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ALGA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Alga profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'alga-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Alga home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_alga_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Alga")
