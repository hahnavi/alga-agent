#!/usr/bin/env python3
"""Google Workspace API CLI for Alga Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Alga-facing JSON contract and falls back to the Python client
...
from _alga_home import get_alga_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ALGA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Alga Agent")
