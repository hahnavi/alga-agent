...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    alga_state = ModuleType("alga_state")
    alga_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "alga_state", alga_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Alga"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Alga"
