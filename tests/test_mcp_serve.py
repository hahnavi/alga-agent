"""
Tests for mcp_serve — Alga MCP server.
...
def _isolate_alga_home(tmp_path, monkeypatch):
    """Redirect ALGA_HOME to a temp directory."""
    monkeypatch.setenv("ALGA_HOME", str(tmp_path))
    try:
        import alga_constants
        monkeypatch.setattr(alga_constants, "get_alga_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking alga_state schema."""
