...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves alga_cli.kanban_db on its own
  ...
  - worker logs are captured to ALGA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ALGA_KANBAN_TASK": task.id,
            "ALGA_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="alga_e2e_")
    os.environ["ALGA_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from alga_cli import kanban_db as kb

    # Point the `alga` CLI child processes will run at the worktree
    # alga_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "alga")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m alga_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/alga_e2e_*/worker_*.log
    import glob
