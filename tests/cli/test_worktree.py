...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("alga/alga-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned alga/* and pr-* branches."""

    def test_prunes_orphaned_alga_branch(self, git_repo):
        """alga/alga-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "alga/alga-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "alga/alga-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "alga/alga-deadbeef" in result.stdout
        ...
        assert "alga/alga-deadbeef" in orphaned
        ...
        assert "alga/alga-deadbeef" not in result.stdout
