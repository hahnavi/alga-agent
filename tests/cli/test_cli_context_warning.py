...
def _isolate(tmp_path, monkeypatch):
    """Isolate ALGA_HOME so tests don't touch real config."""
    home = tmp_path / ".alga"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal AlgaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AlgaCLI.__new__(AlgaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["alga-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Alga' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Alga' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Alga' minimum context length."""
