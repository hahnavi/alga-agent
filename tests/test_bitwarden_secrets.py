...
binary download are exercised manually by `alga secrets bitwarden
...
def alga_home(tmp_path, monkeypatch):
    """Point Alga at an isolated home directory."""
    home = tmp_path / ".alga"
    home.mkdir()
    monkeypatch.setenv("ALGA_HOME", str(home))
    # Some modules cache get_alga_home; clear if needed.
    import alga_constants
    if hasattr(alga_constants, "_ALGA_HOME_CACHE"):
        alga_constants._ALGA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(alga_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(alga_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(alga_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from alga_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under alga_home/cache/."""
    home = tmp_path / ".alga"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
