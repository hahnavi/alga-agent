...
Override the image with ``ALGA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ALGA_TEST_IMAGE", "alga-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ALGA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ALGA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged alga user
# ---------------------------------------------------------------------------
#
# Background: every Alga runtime path inside the container drops to UID
# 10000 (the ``alga`` user) via ``s6-setuidgid alga``. ``docker exec``
...
# fails (PermissionError swallowed) for alga, so a test that ran as root
...
# helpers below run every probe under ``-u alga`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: alga).
