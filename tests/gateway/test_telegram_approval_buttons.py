...
class TestTelegramApprovalCallback:
    ...
    async def test_approval_callback_escapes_dynamic_user_name(self):
        ...
        assert "markdown" in str(edit_kwargs["parse_mode"]).lower() or "markdown" in repr(edit_kwargs["parse_mode"]).lower()
    ...
    async def test_update_prompt_callback_not_affected(self, tmp_path):
        ...
        with patch("tools.approval.resolve_gateway_approval") as mock_resolve:
            with patch("alga_constants.get_alga_home", return_value=tmp_path):
