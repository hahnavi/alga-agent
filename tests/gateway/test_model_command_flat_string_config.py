...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "alga_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_alga_home() / config.yaml`` — point it here.
    monkeypatch.setattr("alga_constants.get_alga_home", lambda: alga_home)
    monkeypatch.setattr("alga_cli.config.get_alga_home", lambda: alga_home)
    return cfg_path
