...
def server():
    with patch.dict(
        "sys.modules",
        {
            "alga_constants": MagicMock(
                get_alga_home=MagicMock(return_value="/tmp/alga_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'alga-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'alga-release' patched"
    }
