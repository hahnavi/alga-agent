...
class TestScanMemoryContent:
    ...
    def test_alga_config_mod_blocked(self):
        result = _scan_memory_content("edit .alga/config.yaml to change settings")
        assert "Blocked" in result
        assert "alga_config_mod" in result
        result = _scan_memory_content("update .alga/SOUL.md with new personality")
        assert "Blocked" in result
        assert "alga_config_mod" in result
    ...
    def test_alga_config_mod_no_false_positives(self):
        """Merely mentioning alga config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .alga/config.yaml for settings") is None
        assert _scan_memory_content("Read .alga/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .alga/config.yaml file contains runtime options") is None
