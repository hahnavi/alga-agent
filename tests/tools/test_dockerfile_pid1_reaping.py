...
- Signal forwarding runs through the init so ``docker stop`` triggers
  alga's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, alga accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, alga accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into alga,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, alga still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``alga-ink`` workspace package is
    ...
    assert "ui-tui/packages/alga-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/alga-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled alga-ink workspace package "
        "so ``await import('@alga/ink')`` resolves at runtime."
    )
