...
def test_check_website_access_uses_dynamic_alga_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new ALGA_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old ALGA_HOME set by the autouse _isolate_alga_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing ALGA_HOME to tmp_path
