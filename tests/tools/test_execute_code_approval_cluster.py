...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ALGA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ALGA_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ALGA_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ALGA_INTERACTIVE", raising=False)
    monkeypatch.delenv("ALGA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ALGA_CRON_SESSION", "1")
    monkeypatch.delenv("ALGA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_alga_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ALGA_HOME": "/h", "ALGA_PROFILE": "p",
        "ALGA_CONFIG": "/c.yaml", "ALGA_ENV": "/e",
        # other ALGA_* → dropped (broad prefix removed)
        "ALGA_BASE_URL": "https://x", "ALGA_INTERACTIVE": "1",
        "ALGA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ALGA_HOME", "ALGA_PROFILE", "ALGA_CONFIG", "ALGA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ALGA_BASE_URL", "ALGA_INTERACTIVE", "ALGA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_alga_vars(caplog):
    """Dropping a non-allowlisted, non-secret ALGA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ALGA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ALGA_HOME": "/h",          # allowlisted → kept, not logged
        "ALGA_BASE_URL": "https://x",   # dropped → logged
        "ALGA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ALGA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ALGA_HOME" in out and "PATH" in out
    assert "ALGA_BASE_URL" not in out and "ALGA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ALGA_BASE_URL" in msgs and "ALGA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ALGA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ALGA_* vars."""
    import logging
