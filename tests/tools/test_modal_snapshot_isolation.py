...
def _restore_tool_modules():
    original_alga_home = os.environ.get("ALGA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_alga_home is None:
            os.environ.pop("ALGA_HOME", None)
        else:
            os.environ["ALGA_HOME"] = original_alga_home
        _reset_modules(("tools", "alga_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    alga_cli = types.ModuleType("alga_cli")
    alga_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["alga_cli"] = alga_cli
    alga_home = tmp_path / "alga-home"
    os.environ["ALGA_HOME"] = str(alga_home)
    sys.modules["alga_cli.config"] = types.SimpleNamespace(
        get_alga_home=lambda: alga_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="alga-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": alga_home / "modal_snapshots.json",
