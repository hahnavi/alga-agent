...
def _clean_state():
    ...
    saved = {}
    for k in ("ALGA_INTERACTIVE", "ALGA_GATEWAY_SESSION", "ALGA_EXEC_ASK", "ALGA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ALGA_INTERACTIVE"] = "1"
        session_key = os.getenv("ALGA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ALGA_INTERACTIVE or ALGA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
