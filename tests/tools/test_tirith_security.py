...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._alga_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(alga_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(alga_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_alga_bin(self):
        """After _INSTALL_FAILED, manual install in ALGA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        alga_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(alga_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(alga_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._alga_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == alga_bin
            assert _tirith_mod._resolved_path == alga_bin
            mock_clear.assert_called_once()
...
class TestAlgaHomeIsolation:
    def test_alga_bin_dir_respects_alga_home(self):
        """_alga_bin_dir must use ALGA_HOME, not hardcoded ~/.alga."""
        from tools.tirith_security import _alga_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ALGA_HOME": tmpdir}):
            result = _alga_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_alga_home(self):
        """_failure_marker_path must use ALGA_HOME, not hardcoded ~/.alga."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ALGA_HOME": "/custom/alga"}):
            result = _failure_marker_path()
        assert result == "/custom/alga/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ALGA_HOME; verify it's active."""
        alga_home = os.getenv("ALGA_HOME")
        assert alga_home is not None, "ALGA_HOME should be set by conftest"
        assert "alga_test" in alga_home, "Should point to test temp dir"

    def test_get_alga_home_fallback(self):
        """Without ALGA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_alga_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ALGA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".alga")
            result = _get_alga_home()
