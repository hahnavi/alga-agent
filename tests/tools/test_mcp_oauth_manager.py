...
def test_alga_provider_subclass_exists():
    """AlgaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ALGA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ALGA_PROVIDER_CLS is not None
    assert issubclass(_ALGA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_alga_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AlgaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ALGA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ALGA_HOME", str(tmp_path))
    ...
    assert _ALGA_PROVIDER_CLS is not None
    assert isinstance(provider, _ALGA_PROVIDER_CLS)
    assert provider._alga_server_name == "srv"
