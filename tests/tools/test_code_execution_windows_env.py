...
``alga_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ALGA_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # ALGA_* handling (#27303): only the operational allowlist passes;
        # every other ALGA_* is dropped (the broad prefix was removed).
        "ALGA_HOME": "/home/alice/.alga",        # allowlisted → kept
        "ALGA_PROFILE": "default",                 # allowlisted → kept
        "ALGA_INTERACTIVE": "1",                   # not allowlisted → dropped
        "ALGA_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "ALGA_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``alga_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``alga_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_alga_tools_module
        stub = generate_alga_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_alga_tools_module
        import tempfile, ast
        stub = generate_alga_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_alga_tools_module(["terminal"], transport="uds")
...
# let the child import alga_tools, a user script that printed non-ASCII
