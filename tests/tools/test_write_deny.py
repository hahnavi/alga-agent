...
class TestWriteDenyExactPaths:
    ...
    def test_alga_env(self):
        # ``.env`` under the active ALGA_HOME (profile-aware, not just
        # ``~/.alga``) must be write-denied. The hermetic test conftest
        # points ALGA_HOME at a tempdir — resolve via get_alga_home()
        # to match the denylist.
        from alga_constants import get_alga_home
        path = str(get_alga_home() / ".env")
        assert _is_write_denied(path) is True

    def test_alga_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.alga/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "alga_root"
        ...
        # Sanity check: ALGA_HOME does point to the profile dir, not the root.
        from alga_constants import get_alga_home, get_default_alga_root
        assert get_alga_home() == profile_home
        assert get_default_alga_root() == root
...
class TestWriteAllowed:
    ...
    def test_alga_config_not_env(self):
        path = os.path.join(str(Path.home()), ".alga", "config.yaml")
