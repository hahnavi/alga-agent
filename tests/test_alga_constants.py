"""Tests for alga_constants module."""
...
class TestGetDefaultAlgaRoot:
    """Tests for get_default_alga_root() — Docker/custom deployment awareness."""

    def test_no_alga_home_returns_native(self, tmp_path, monkeypatch):
        """When ALGA_HOME is not set, returns ~/.alga."""
        monkeypatch.delenv("ALGA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_alga_root() == tmp_path / ".alga"

    def test_alga_home_is_native(self, tmp_path, monkeypatch):
        """When ALGA_HOME = ~/.alga, returns ~/.alga."""
        native = tmp_path / ".alga"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALGA_HOME", str(native))
        assert get_default_alga_root() == native

    def test_alga_home_is_profile(self, tmp_path, monkeypatch):
        """When ALGA_HOME is a profile under ~/.alga, returns ~/.alga."""
        native = tmp_path / ".alga"
    ...
    def test_alga_home_is_docker(self, tmp_path, monkeypatch):
        """When ALGA_HOME points outside ~/.alga (Docker), returns ALGA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALGA_HOME", str(docker_home))
        assert get_default_alga_root() == docker_home

    def test_alga_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ALGA_HOME outside ~/.alga is treated as the root."""
        custom = tmp_path / "my-alga-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALGA_HOME", str(custom))
        assert get_default_alga_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ALGA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ALGA_HOME", str(profile))
        assert get_default_alga_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(alga_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(alga_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".alga"
