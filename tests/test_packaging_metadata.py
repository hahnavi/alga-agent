...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``alga_cli``) for the package
    itself and ``alga_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``alga_cli`` without ``alga_cli.*``,
    so the wheel shipped ``alga_cli/*.py`` but dropped the ``proxy``
    subpackage. A user-importing module from that subpackage then died on every
    install with ``ModuleNotFoundError: No module named 'alga_cli.proxy'``.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (alga_cli/plugins.py) registers each bundled plugin by
