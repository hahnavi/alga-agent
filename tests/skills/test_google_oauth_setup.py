...
class TestAlgaConstantsFallback:
    """Tests for _alga_home.py fallback when alga_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_alga_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _alga_home.py with alga_constants blocked."""
        monkeypatch.setitem(sys.modules, "alga_constants", None)
        spec = importlib.util.spec_from_file_location("_alga_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_alga_home_env_var(self, monkeypatch, tmp_path):
        """When alga_constants is missing, ALGA_HOME comes from env var."""
        monkeypatch.setenv("ALGA_HOME", str(tmp_path / "custom-alga"))
        module = self._load_helper(monkeypatch)
        assert module.get_alga_home() == tmp_path / "custom-alga"

    def test_fallback_defaults_to_dot_alga(self, monkeypatch):
        """When alga_constants is missing and ALGA_HOME unset, default to ~/.alga."""
        monkeypatch.delenv("ALGA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_alga_home() == Path.home() / ".alga"

    def test_fallback_ignores_empty_alga_home(self, monkeypatch):
        """Empty/whitespace ALGA_HOME is treated as unset."""
        monkeypatch.setenv("ALGA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_alga_home() == Path.home() / ".alga"

    def test_fallback_display_alga_home_shortens_path(self, monkeypatch):
        """Fallback display_alga_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ALGA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_alga_home() == "~/.alga"

    def test_fallback_display_alga_home_profile_path(self, monkeypatch):
        """Fallback display_alga_home() handles profile paths under ~/."""
        monkeypatch.setenv("ALGA_HOME", str(Path.home() / ".alga/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_alga_home() == "~/.alga/profiles/coder"

    def test_fallback_display_alga_home_custom_path(self, monkeypatch):
        """Fallback display_alga_home() returns full path for non-home locations."""
        monkeypatch.setenv("ALGA_HOME", "/opt/alga-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_alga_home() == "/opt/alga-custom"

    def test_delegates_to_alga_constants_when_available(self):
        """When alga_constants IS importable, _alga_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_alga_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import alga_constants
        assert module.get_alga_home is alga_constants.get_alga_home
        assert module.display_alga_home is alga_constants.display_alga_home
