"""Tests for alga_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Alga entry point
(alga, alga-agent, alga-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Alga entry point imports alga_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported alga_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("alga_bootstrap", None)
    import alga_bootstrap  # noqa: WPS433
    return alga_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Alga CLI
...
class TestEntryPointsImportBootstrap:
    """Every Alga entry point must import alga_bootstrap as its
    ...
    # Entry points that invoke Alga as a process.  Each one must
    # import alga_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "alga_cli/main.py",   # alga CLI (console_script)
        "run_agent.py",          # alga-agent (console_script)
        "acp_adapter/entry.py",  # alga-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import alga_bootstrap' and that
        ...
        half-finished ``alga update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``alga_bootstrap`` as a top-level module) leaves alga
        ...
        """
        # Resolve relative to the alga-agent repo root.  Tests live
        # at tests/test_alga_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # alga start even when alga_bootstrap hasn't been
        ...
        assert first_import_name == "alga_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'alga_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import alga_bootstrap' line to be the first import."
        )
