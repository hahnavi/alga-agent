...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"alga-agent[termux]"' in text
    assert '"alga-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"alga-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
