...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_alga_env(self):
        """Alga' own .env inside ALGA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: alga internal .env is in ~/.alga/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.alga/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.alga/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Alga cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        alga_home = tmp_path / ".alga"
        cache = alga_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._alga_home_path", return_value=alga_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Alga cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        alga_home = tmp_path / ".alga"
        hub = alga_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_alga_home(self, tmp_path):
        """The env basename guard does not depend on ALGA_HOME resolution."""
        alga_home = tmp_path / ".alga"
        alga_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        alga_home = tmp_path / ".alga"
        cache = alga_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
