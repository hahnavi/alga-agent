...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ALGA_HOME under tmp_path."""
    monkeypatch.setenv("ALGA_HOME", str(tmp_path / ".alga"))
    (tmp_path / ".alga").mkdir()

    # Force the constants/image cache helpers to re-read ALGA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("alga_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_alga_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ALGA_HOME — gateway cleanup
