...
to a generic "Your API key was rejected... run alga setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "alga setup".
    assert "alga auth add nous --type oauth" in source
