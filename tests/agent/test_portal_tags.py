...
def test_alga_client_tag_includes_current_version():
    """The client tag must reflect alga_cli.__version__ verbatim."""
    from alga_cli import __version__
    from agent.portal_tags import alga_client_tag

    assert alga_client_tag() == f"client=alga-client-v{__version__}"


def test_alga_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import alga_client_tag

    tag = alga_client_tag()
    assert tag.startswith("client=alga-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import alga_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=alga-agent" in tags
    assert alga_client_tag() in tags
    assert len(tags) == 2
