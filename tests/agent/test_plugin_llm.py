...
""",
        ...
        from alga_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        alga_home = tmp_path / ".alga"
        alga_home.mkdir()
        (alga_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ALGA_HOME", str(alga_home))
        from alga_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from alga_cli.plugins import PluginContext, PluginManifest, PluginManager
