...
engine plugins (e.g. alga-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``alga plugins`` picker."""
    from alga_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "alga_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "alga_cli.plugins.get_plugin_context_engine",
