...
def isolated_home(monkeypatch):
    """Temp ALGA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="alga_test_31179_")
    alga_home = os.path.join(test_home, ".alga")
    os.makedirs(alga_home)
...
def _fresh_modules():
    """Drop cached alga modules so each test reloads against current env."""
