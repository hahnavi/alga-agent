...
def curator_env(tmp_path, monkeypatch):
    """Isolated ALGA_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".alga"
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `alga curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """alga curator unpin must refuse bundled/hub skills too (matches pin)."""
    from alga_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `alga model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    Two sources of truth remain after the web UI dashboard was removed:
    the schema source (DEFAULT_CONFIG.auxiliary) and the CLI picker
    (_AUX_TASKS). The old web_server._AUX_TASK_SLOTS and
    web/src/pages/ModelsPage.tsx were tied to the (now removed) web UI.
    """
    from alga_cli.config import DEFAULT_CONFIG
    from alga_cli.main import _AUX_TASKS
    ...
    # 2. alga_cli/main.py _AUX_TASKS — CLI picker
