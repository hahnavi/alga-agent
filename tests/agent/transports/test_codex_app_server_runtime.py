...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Alga passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ALGA_HOME", "/users/alice/.alga/profiles/backend-worker")
        monkeypatch.setenv("ALGA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ALGA_KANBAN_DB",
            "/users/alice/.alga/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.alga/kanban/boards/smoke"]'
            in cmd
        )
