...
- Path resolution (absolute, relative to ALGA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ALGA_HOME."""
    alga_home = tmp_path / ".alga"
    alga_home.mkdir()
    (alga_home / "cron").mkdir()
    (alga_home / "cron" / "output").mkdir()
    (alga_home / "scripts").mkdir()
    monkeypatch.setenv("ALGA_HOME", str(alga_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ALGA_DIR", alga_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", alga_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", alga_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", alga_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.alga/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ALGA_SESSION_PLATFORM") is None
        assert os.environ.get("ALGA_SESSION_CHAT_ID") is None
        assert os.environ.get("ALGA_SESSION_CHAT_NAME") is None
