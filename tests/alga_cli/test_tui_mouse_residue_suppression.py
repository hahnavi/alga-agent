...
The Python launcher (`alga --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Alga session
...
from alga_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["alga", "--tui", "-c", "abc"])
        monkeypatch.delenv("ALGA_TUI", raising=False)
        monkeypatch.delenv("ALGA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_alga_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["alga"])
        monkeypatch.setenv("ALGA_TUI", "1")
        monkeypatch.delenv("ALGA_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("ALGA_TUI", raising=False)
        monkeypatch.setenv("ALGA_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `alga --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
