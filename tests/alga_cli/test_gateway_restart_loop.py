...
Covers:
- Defense 1: gateway stop/restart refuse when _ALGA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.alga.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.alga.gateway.plist",
        "launchctl stop ai.alga.gateway",
        "systemctl restart alga-gateway",
        "systemctl stop alga-gateway.service",
        "systemctl start alga-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill alga gateway process",
        "pkill -f alga.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_alga_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade alga then run alga gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.alga.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify alga gateway stop/restart refuse when _ALGA_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_ALGA_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ALGA_GATEWAY", raising=False)
