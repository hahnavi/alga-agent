"""Tests for the `alga memory reset` CLI command.
...
- Profile-scoped reset (uses ALGA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ALGA_HOME with memory files."""
    alga_home = tmp_path / ".alga"
    memories = alga_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ALGA_HOME", str(alga_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAlga repo is at ~/.alga/alga-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `alga memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        alga_home = tmp_path / ".alga"
        (alga_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ALGA_HOME", str(alga_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ALGA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_alga_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
