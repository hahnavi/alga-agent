"""Tests for alga_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ALGA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Alga-managed node dir.
"""
...
import alga_cli.uninstall as uninstall
...
def _make_alga_node(alga_home: Path) -> Path:
    """Create a fake $ALGA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = alga_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_alga_node(fake_home):
    alga_home = fake_home / ".alga"
    node_bin = _make_alga_node(alga_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(alga_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(alga_home) == []


def test_removes_dangling_symlink_into_alga_node(fake_home):
    """A link into the Alga node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Alga links that exist; ignores the ones that don't."""
    alga_home = fake_home / ".alga"
    node_bin = _make_alga_node(alga_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Alga-managed; node is a real user binary.
