"""Tests for IRC gateway configuration via `alga setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Alga install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Alga install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#alga")
            monkeypatch.setenv("IRC_NICKNAME", "alga-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `alga setup gateway` experience with IRC present."""
