"""Tests for Codex auth — tokens stored in Alga auth store (~/.alga/auth.json)."""
...
def _setup_alga_auth(alga_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Alga auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_alga_auth(alga_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ALGA_HOME", str(alga_home))
    ...
    monkeypatch.setattr("alga_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    alga_home = tmp_path / "alga"
    _setup_alga_auth(alga_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ALGA_HOME", str(alga_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((alga_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``alga auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Alga auth store, not ~/.codex/."""
    alga_home = tmp_path / "alga"
    codex_home = tmp_path / "codex-cli"
    alga_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "alga-at", "refresh_token": "alga-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Alga store
    assert not (codex_home / "auth.json").exists()

    # Alga auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "alga-at"


def test_resolve_returns_alga_auth_store_source(tmp_path, monkeypatch):
    alga_home = tmp_path / "alga"
    _setup_alga_auth(alga_home)
    monkeypatch.setenv("ALGA_HOME", str(alga_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run alga auth".
    """
    from alga_cli.auth import (
    ...
    assert "alga auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from alga_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from alga_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "alga_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("alga_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("alga_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
