...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ALGA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ALGA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ALGA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from alga_cli.providers import ALGA_OVERLAYS
        assert "bedrock" in ALGA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from alga_cli.providers import ALGA_OVERLAYS
        assert ALGA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from alga_cli.providers import ALGA_OVERLAYS
        assert ALGA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
