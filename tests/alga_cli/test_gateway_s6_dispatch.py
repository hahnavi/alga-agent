"""Tests for the Phase 4 s6 dispatch helper in alga_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`alga gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from alga_cli import gateway as gw
    monkeypatch.setattr(
        "alga_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "alga_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "alga_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "alga_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`alga -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from alga_cli import gateway as gw
    from alga_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "alga_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "alga profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "alga_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "alga_cli.service_manager.detect_service_manager",
...
def _stub_execvp(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    monkeypatch.setattr("alga_cli.gateway.os.execvp", fake_execvp)
    return calls
...
def test_redirect_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "alga_cli.gateway.os.execvp",
        lambda *a, **kw: pytest.fail("execvp should not be called on host"),
    )
    monkeypatch.delenv("ALGA_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("ALGA_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("alga_cli.gateway._profile_suffix", lambda: "")
    ...
    assert "ALGA_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `alga gateway run --replace`, the
    ALGA_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
    ...
    monkeypatch.setenv("ALGA_S6_SUPERVISED_CHILD", "1")
    monkeypatch.delenv("ALGA_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_respects_no_supervise_env(
    ...
    """`ALGA_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("ALGA_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("ALGA_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of ALGA_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `ALGA_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        monkeypatch.setenv("ALGA_GATEWAY_NO_SUPERVISE", falsy)
