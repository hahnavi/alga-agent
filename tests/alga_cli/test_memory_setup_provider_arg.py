"""Tests for `alga memory setup [provider]` routing.
...
``alga memory setup honcho``) without the interactive picker — which matters
because the per-provider ``alga <provider>`` subcommand is only registered
...
from alga_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "alga memory setup" in out
