"""Tests for ``alga update`` / ``--check`` inside the Docker container.
...
fix, ``alga update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Alga, not an update to the running container, so the message
...
from alga_cli.main import _cmd_update_check, cmd_update


# ---------- cmd_update (apply path) ----------


@patch("alga_cli.config.is_managed", return_value=False)
@patch("alga_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``alga update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull hahnavi/alga-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``alga update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull hahnavi/alga-agent:latest" in msg
    ...
    assert "ALGA_HOME" in msg or "/opt/data" in msg, (
