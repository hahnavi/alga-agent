...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ALGA_HOME with .container-mode file."""
    alga_home = tmp_path / ".alga"
    alga_home.mkdir()
    monkeypatch.setenv("ALGA_HOME", str(alga_home))
    monkeypatch.delenv("ALGA_DEV", raising=False)

    container_mode = alga_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=alga-agent\n"
        "exec_user=alga\n"
        "alga_bin=/data/current-package/bin/alga\n"
    )
    return alga_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("alga_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "alga-agent"
    assert info["exec_user"] == "alga"
    assert info["alga_bin"] == "/data/current-package/bin/alga"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("alga_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_alga_dev(container_env, monkeypatch):
    """Returns None when ALGA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ALGA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_alga_dev_zero(container_env, monkeypatch):
    """ALGA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ALGA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        alga_home = Path(tmpdir) / ".alga"
        alga_home.mkdir()
        (alga_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("alga_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_alga_home": lambda: alga_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ALGA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=alga-custom\n"
        "exec_user=myuser\n"
        "alga_bin=/opt/alga/bin/alga\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "alga-custom"
    assert info["exec_user"] == "myuser"
    assert info["alga_bin"] == "/opt/alga/bin/alga"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from alga_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "alga"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "alga-agent" in cmd
    assert "/data/current-package/bin/alga" in cmd
    assert "chat" in cmd
