"""Tests for the kanban CLI surface (alga_cli.kanban)."""
...
def test_run_slash_session_filter(kanban_home):
    """`alga kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from alga_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("ALGA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ALGA_KANBAN_BOARD") == "beta"
