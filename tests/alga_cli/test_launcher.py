"""Tests for the top-level `./alga` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./alga` should use `alga_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "alga"
    called = []

    fake_main_module = types.ModuleType("alga_cli.main")

    def fake_main():
        called.append("alga_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "alga_cli.main", fake_main_module)
