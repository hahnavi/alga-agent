...
``alga_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``alga --help``, ``alga version``,
``alga logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``alga --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from alga_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["alga", "-w", "--tui"], None),
        (["alga", "version"], "version"),
        (["alga", "--tui", "chat"], "chat"),
        (["alga", "-w", "logs"], "logs"),
        (["alga", "chat", "hello world"], "chat"),
        (["alga", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["alga", "-m", "gpt5", "chat"], "chat"),
        (["alga", "--model", "gpt5", "chat", "hi"], "chat"),
        (["alga", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["alga", "-z", "hello world"], None),
        (["alga", "-z", "hello", "chat"], "chat"),
        (["alga", "--model=gpt5", "chat"], "chat"),     # inline form
        (["alga", "--", "chat"], "chat"),               # -- terminator
        (["alga", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["alga", "some-plugin-cmd"], "some-plugin-cmd"),
        (["alga", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["alga"],                          # bare → chat
        ["alga", "--help"],                # top-level help
        ["alga", "-h"],
        ["alga", "version"],               # known built-in
        ["alga", "logs"],
        ["alga", "gateway", "run"],
        ["alga", "--tui"],
        ["alga", "-w", "--tui"],
        ["alga", "chat", "hi"],
        ["alga", "help"],                  # accepted built-in-ish
        ["alga", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["alga", "meet", "join"],          # potential google_meet plugin
        ["alga", "honcho", "status"],      # potential memory plugin
        ["alga", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``alga help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"alga_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
