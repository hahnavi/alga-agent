"""Tests for alga_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ALGA_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".alga"
    home.mkdir()
    ...
    import importlib
    from alga_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from alga_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://alga-agent.hahnavi.my.id/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/hahnavi/alga-agent"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in alga_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from alga_cli import model_catalog
        from alga_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from alga_cli import model_catalog
        from alga_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``alga model`` CLI — not the live
        ...
        # ``ALGA_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ALGA_HOME directly instead.
        import importlib
        from alga_cli import model_catalog
        from alga_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from alga_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["ALGA_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("alga_cli.models.check_nous_free_tier", return_value=False), patch(
                "alga_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "alga_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
