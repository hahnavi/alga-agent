"""Regression tests for #34107 — Docker UID/GID handling in ensure_alga_home.

When Alga runs in Docker with ``ALGA_UID=1000`` / ``ALGA_GID=911``,
the entrypoint chowns the top-level ``ALGA_HOME`` once at startup. But
...
The fix is a ``_chown_to_alga_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_alga_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAlgaUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ALGA_UID", "1000")
        monkeypatch.setenv("ALGA_GID", "911")
        from alga_cli.config import _resolve_alga_uid_gid
        uid, gid = _resolve_alga_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ALGA_UID", raising=False)
        monkeypatch.delenv("ALGA_GID", raising=False)
        from alga_cli.config import _resolve_alga_uid_gid
        uid, gid = _resolve_alga_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ALGA_UID", "not-a-number")
        monkeypatch.setenv("ALGA_GID", "911")
        from alga_cli.config import _resolve_alga_uid_gid
        uid, gid = _resolve_alga_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ALGA_UID", "")
        monkeypatch.setenv("ALGA_GID", "")
        from alga_cli.config import _resolve_alga_uid_gid
        uid, gid = _resolve_alga_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToAlgaUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_alga_uid(d)  # must not raise
