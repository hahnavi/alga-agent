...
from alga_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from alga_cli.config import load_config, save_config, save_env_value
from alga_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from alga_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside alga setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("alga_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("alga_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("alga_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'alga setup' to configure" not in output
