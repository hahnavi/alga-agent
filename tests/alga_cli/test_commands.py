...
class TestSlackNativeSlashes:
    ...
    def test_alga_catchall_is_first(self):
        """``/alga`` must be reserved as the first slot so the legacy
        ``/alga <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "alga"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /alga <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `alga slack manifest`)."""
...
from alga_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch alga_cli.plugins.get_plugin_commands() to a fixed dict."""
        from alga_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/alga metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
    ...
    def test_is_gateway_known_command_recognizes_plugin_commands(self, monkeypatch):
        ...
        from alga_cli.commands import is_gateway_known_command
