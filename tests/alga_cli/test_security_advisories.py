"""Tests for alga_cli.security_advisories.
...
import alga_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ALGA_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".alga"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full alga_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "alga_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "alga_cli.config.save_config",
