"""Tests for alga_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from alga_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("alga_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from alga_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("alga_cli.config.load_config")
    ...
    @patch("alga_cli.config.load_config")
    @patch.dict("os.environ", {"ALGA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ALGA_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ALGA_SESSION_PLATFORM should be used when ALGA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_alga_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ALGA_PLATFORM should win over ALGA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("ALGA_HOME", str(tmp_path))
        monkeypatch.setenv("ALGA_PLATFORM", "telegram")
        monkeypatch.setenv("ALGA_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ALGA_HOME", str(tmp_path))
        monkeypatch.setenv("ALGA_PLATFORM", "telegram")
        monkeypatch.setenv("ALGA_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from alga_cli.skills_config import _get_categories
        skills = [
