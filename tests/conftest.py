"""Shared fixtures for the alga-agent test suite.
...
2. **Isolated ALGA_HOME.** ALGA_HOME points to a per-test tempdir so
   code reading ``~/.alga/*`` via ``get_alga_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".alga"`` instead of the canonical
   ``get_alga_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ALGA_SESSION_* inheritance** — the agent's current gateway
...
# ALGA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ALGA_BEHAVIORAL_VARS = frozenset({
    "ALGA_YOLO_MODE",
    "ALGA_INTERACTIVE",
    "ALGA_QUIET",
    "ALGA_TOOL_PROGRESS",
    ...
    "ALGA_SESSION_CHAT_ID",
    "ALGA_SESSION_CHAT_NAME",
    "ALGA_SESSION_THREAD_ID",
    "ALGA_SESSION_SOURCE",
    "ALGA_SESSION_KEY",
    "ALGA_GATEWAY_SESSION",
    "_ALGA_GATEWAY",
    "ALGA_PLATFORM",
    "ALGA_MODEL",
    "ALGA_INFERENCE_MODEL",
    "ALGA_INFERENCE_PROVIDER",
    "ALGA_TUI_PROVIDER",
    "ALGA_MANAGED",
    "ALGA_DEV",
    "ALGA_CONTAINER",
    ...
    "ALGA_TIMEZONE",
    "ALGA_REDACT_SECRETS",
    "ALGA_BACKGROUND_NOTIFICATIONS",
    "ALGA_EXEC_ASK",
    "ALGA_HOME_MODE",
    "ALGA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.alga/kanban.db instead of the per-test ALGA_HOME.
    "ALGA_KANBAN_DB",
    "ALGA_KANBAN_BOARD",
    "ALGA_KANBAN_HOME",
    "ALGA_KANBAN_WORKSPACES_ROOT",
    "ALGA_KANBAN_LOGS_ROOT",
    "ALGA_KANBAN_TASK",
    "ALGA_KANBAN_WORKSPACE",
    "ALGA_KANBAN_RUN_ID",
    "ALGA_KANBAN_CLAIM_LOCK",
    "ALGA_KANBAN_DISPATCH_IN_GATEWAY",
    "ALGA_TENANT",
    ...
    "ALGA_DASHBOARD_OAUTH_CLIENT_ID",
    "ALGA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ALGA_HOME to per-test tempdirs so code that
    reads ``~/.alga/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ALGA_* vars that could change test semantics.
    for name in _ALGA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ALGA_HOME to a per-test tempdir. Code that reads
    #    ``~/.alga/*`` via ``get_alga_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.alga/*`` via
    #    ``Path.home() / ".alga"`` instead of ``get_alga_home()``
    #    is a bug to fix at the callsite.
    fake_alga_home = tmp_path / "alga_test"
    fake_alga_home.mkdir()
    (fake_alga_home / "sessions").mkdir()
    (fake_alga_home / "cron").mkdir()
    (fake_alga_home / "memories").mkdir()
    (fake_alga_home / "skills").mkdir()
    monkeypatch.setenv("ALGA_HOME", str(fake_alga_home))
    ...
    #    ~/.alga/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import alga_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_alga_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal alga config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``alga-gateway`` process
...
# correlated with a ``tests/alga_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> alga-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart alga-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting alga/python patterns are also blocked.
    """
    ...
    def _matches_alga_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ALGA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_alga_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch alga-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m alga_cli.main".
                if (
                    "alga" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live alga-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting alga/python could hit the live gateway. "
