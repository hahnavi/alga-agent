import type { RunExternalProcess } from '@alga/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`alga ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching alga: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`alga ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
