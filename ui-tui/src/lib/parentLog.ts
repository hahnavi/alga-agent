...
const logDir = join(process.env.ALGA_HOME?.trim() || join(homedir(), '.alga'), 'logs')
...
// a real ~/.alga (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('alga-tui: parent lifecycle log unavailable\n')
    }
  }
}
