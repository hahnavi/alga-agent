...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ALGA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Alga on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ALGA_TUI_TRUECOLOR=1.
