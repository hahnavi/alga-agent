...
React + Ink terminal UI for Alga. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ALGA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.alga/.alga_history` or under `ALGA_HOME`.
...
```text
ui-tui/
  packages/alga-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      alga-ink.d.ts    type declarations for @alga/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AlgaCLI subprocess for slash commands
```
