...
await build({
  ...
  // Skip the prebuilt @alga/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@alga/ink': resolve(root, 'packages/alga-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The alga_cli launcher
