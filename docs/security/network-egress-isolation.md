...
When running Alga inside Docker, the default `network_mode: host` gives the
...
The Alga [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ALGA_UID=$(id -u) ALGA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=alga,alga-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://alga-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Alga trust model and vulnerability reporting
