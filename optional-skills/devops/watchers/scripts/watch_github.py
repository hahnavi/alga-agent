...
Usage (via cron with --no-agent):
    ...
    alga cron create alga-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ALGA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name alga-issues --repo hahnavi/alga-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.alga/.env to avoid the 60 req/hr
