...
Usage (via cron with --no-agent):
    ...
    alga cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ALGA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Alga-Watcher/1.0"})
