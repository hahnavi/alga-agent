"""Centralized logging setup for Alga Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.alga/logs/`` (profile-aware via ``get_alga_home()``).
...
from alga_constants import get_config_path, get_alga_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_alga_session_injector", False):
    ...
    _session_record_factory._alga_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``alga logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "alga_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Alga logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    alga_home
        Override for the Alga home directory.  Falls back to
        ``get_alga_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_alga_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._alga_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the alga group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from alga_cli.config import is_managed
