# nix/nixosModules.nix — NixOS module for alga-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: alga runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ALGA_HOME/.env
# and read by alga at startup — no container recreation needed for env changes.
#
# Tool resolution: the alga wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.alga-agent;
    effectivePackage =
    ...
    alga-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-alga-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "alga-config-attrs";
      description = "Alga YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "alga-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "alga-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ALGA_DOC_EOF'\n${value}\nALGA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "alga-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/alga";
    ...
    # alga user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "alga-container-entrypoint" ''
      set -eu

      ALGA_UID="''${ALGA_UID:?ALGA_UID must be set}"
      ALGA_GID="''${ALGA_GID:?ALGA_GID must be set}"

      # ── Group: ensure a group with GID=$ALGA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ALGA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="alga"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ALGA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ALGA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ALGA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ALGA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="alga"
        TARGET_HOME="/home/alga"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ALGA_UID" -g "$ALGA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ALGA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ALGA_UID:$ALGA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ALGA_HOME is owned by the target user.
      ...
      if [ -n "''${ALGA_HOME:-}" ] && [ -d "$ALGA_HOME" ]; then
        find "$ALGA_HOME" \! -user "$ALGA_UID" -exec chown "$ALGA_UID:$ALGA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/alga-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/alga-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/alga ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/alga
        chmod 0440 /etc/sudoers.d/alga
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ALGA_UID" --regid="$ALGA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ALGA_HOME/.env.
    ...
    # Default: /var/lib/alga/workspace → /data/workspace.
  ...
  in {
    options.services.alga-agent = with lib; {
      enable = mkEnableOption "Alga Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = alga-agent;
        description = "The alga-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/alga";
        description = "State directory. Contains .alga/ subdir (ALGA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Alga config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ALGA_HOME/.env at activation time.
          Alga reads this file on every startup via load_alga_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ALGA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ALGA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `alga gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the alga user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the alga plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Alga discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          alga_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as alga (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in alga-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the alga CLI to environment.systemPackages and export
          ALGA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.alga symlink to the service
            stateDir. These users are automatically added to the alga group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.alga-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the alga CLI to system PATH and export ALGA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.alga/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ALGA_HOME = "${cfg.stateDir}/.alga";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.alga-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the alga user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.alga-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed alga
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure alga is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.alga        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alga/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alga/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alga/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alga/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.alga/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."alga-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.alga
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.alga ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.alga ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.alga -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.alga/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.alga/$_subdir"
            chmod 2770 "${cfg.stateDir}/.alga/$_subdir"
            find "${cfg.stateDir}/.alga/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.alga/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.alga/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alga/config.yaml
            chmod 0640 ${cfg.stateDir}/.alga/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.alga/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alga/.managed
          chmod 0644 ${cfg.stateDir}/.alga/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.alga/.container-mode <<'ALGA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    alga_bin=${containerDataDir}/current-package/bin/alga
    ALGA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alga/.container-mode
            chmod 0644 ${cfg.stateDir}/.alga/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.alga/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alga";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.alga" ]; then
                  rm -f "${symlinkPath}"
                  echo "alga-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.alga -> stateDir/.alga for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.alga";
                target = "${cfg.stateDir}/.alga";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "alga-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alga/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.alga/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.alga/auth.json
              fi
            ''}
          ''}
          ...
          # Alga reads $ALGA_HOME/.env at startup via load_alga_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.alga/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ALGA_NIX_ENV_EOF'
    ${envFileContent}
    ALGA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.alga/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.alga/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.alga/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.alga-agent = {
          description = "Alga Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ALGA_HOME/.env by the activation script. load_alga_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/alga"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the alga group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.alga-agent = {
          description = "Alga Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ALGA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ALGA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ALGA_UID="$ALGA_UID" \
                --env ALGA_GID="$ALGA_GID" \
                --env ALGA_HOME=${containerDataDir}/.alga \
                --env ALGA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/alga gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
