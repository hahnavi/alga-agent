...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      alga-agent = self'.packages.default;
      algaVenv = alga-agent.algaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "alga-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${algaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "alga-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "alga-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${alga-agent}/bin/alga || (echo "FAIL: alga binary missing"; exit 1)
          test -x ${alga-agent}/bin/alga-agent || (echo "FAIL: alga-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${alga-agent}/bin/alga version 2>&1 | grep -qi "alga" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "alga-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in alga alga-agent alga-acp; do
            test -x ${alga-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "alga-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking alga --help ==="
          ${alga-agent}/bin/alga --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${alga-agent}/bin/alga --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "alga-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${alga-agent}/share/alga-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${alga-agent}/share/alga-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ALGA_BUNDLED_SKILLS" ${alga-agent}/bin/alga || \
            (echo "FAIL: ALGA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ALGA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "alga-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${alga-agent}/share/alga-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${alga-agent}/share/alga-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ALGA_BUNDLED_PLUGINS" ${alga-agent}/bin/alga || \
            (echo "FAIL: ALGA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ALGA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "alga-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${alga-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${alga-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ALGA_TUI_DIR" ${alga-agent}/bin/alga || \
            (echo "FAIL: ALGA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ALGA_TUI_DIR set in wrapper"
        ...
        # Verify ALGA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        alga-node = pkgs.runCommand "alga-node-version" { } ''
          set -e
          echo "=== Checking ALGA_NODE in wrapper ==="
          grep -q "ALGA_NODE" ${alga-agent}/bin/alga || \
            (echo "FAIL: ALGA_NODE not set in wrapper"; exit 1)
          echo "PASS: ALGA_NODE present in wrapper"

          ALGA_NODE=$(sed -n "s/^export ALGA_NODE='\(.*\)'/\1/p" ${alga-agent}/bin/alga)
          test -x "$ALGA_NODE" || (echo "FAIL: ALGA_NODE=$ALGA_NODE not executable"; exit 1)
          echo "PASS: ALGA_NODE executable at $ALGA_NODE"

          NODE_MAJOR=$("$ALGA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ALGA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ALGA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "alga-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ALGA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ALGA_MANAGED guards ==="
          check_blocked "config set" ${alga-agent}/bin/alga config set model foo
          check_blocked "config edit" ${alga-agent}/bin/alga config edit
        ...
        extra-python-packages = let
          ...
          algaWithExtra = alga-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "alga-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${algaWithExtra}/bin/alga || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${algaWithExtra}/bin/alga || \
          ...
          if grep -q "PYTHONPATH" ${alga-agent}/bin/alga; then
        ...
        extra-dependency-groups = let
          algaWithGroups = alga-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "alga-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${algaWithGroups}"
          echo "venv: ${algaWithGroups.algaVenv}"
        ...
        messaging-variant = pkgs.runCommand "alga-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.algaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "alga-config-roundtrip" {
          ...
          merge_and_load() {
            local alga_home="$1"
            export ALGA_HOME="$alga_home"
            ${configMergeScript} ${nixSettings} "$alga_home/config.yaml"
            ${algaVenv}/bin/python3 -c '
import json, sys
