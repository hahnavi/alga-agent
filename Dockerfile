...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/alga/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when alga
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# alga process and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main alga process and
...
RUN set -eu; \
    ...
    # 'Alga WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via ALGA_UID at runtime
RUN useradd -u 10000 -m -d /opt/data alga
...
#
# ui-tui/packages/alga-ink/ is copied IN FULL (not just its manifests)
# because it is referenced as a `file:` workspace dependency from
# ui-tui/package.json.  Copying the tree up front lets npm resolve the
# workspace to real content instead of stopping at a bare package.json.
COPY package.json package-lock.json ./
COPY ui-tui/package.json ui-tui/package-lock.json ui-tui/
COPY ui-tui/packages/alga-ink/ ui-tui/packages/alga-ink/
...
# lock on the @alga/ink entry, tripped the TUI launcher's
...
COPY --chown=alga:alga . .

# Build the terminal UI assets.
RUN cd ui-tui && npm run build

# ---------- Permissions ----------
# Make install dir world-readable so any ALGA_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the alga user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# alga_cli/main.py succeeds (see #18800).
# The .venv MUST remain alga-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/alga && \
    chown -R alga:alga /opt/alga/.venv /opt/alga/ui-tui /opt/alga/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the alga user via
# `s6-setuidgid alga` in its run script. If ALGA_UID is unset, services
# run as the default alga user (UID 10000).

# ---------- Link alga-agent itself (editable) ----------
...
# container always returns nothing — meaning `alga dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the ALGA_GIT_SHA build-arg to
# /opt/alga/.alga_build_sha at build time, and have
# alga_cli/build_info.py read it at runtime.  Both `alga dump` and
...
ARG ALGA_GIT_SHA=
RUN if [ -n "${ALGA_GIT_SHA}" ]; then \
        printf '%s\n' "${ALGA_GIT_SHA}" > /opt/alga/.alga_build_sha && \
        chown alga:alga /opt/alga/.alga_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-alga. (The web UI
# dashboard used to ship as a second static service, but the dashboard
# was removed.)
...
# `exec alga`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $ALGA_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/alga/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-alga-setup && \
    chmod +x /etc/cont-init.d/01-alga-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV ALGA_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> alga ...` they default to root, and any file the
# command writes under $ALGA_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/alga/bin/alga, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid alga`
# when invoked as root. Non-root callers (supervised processes,
# `--user alga`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/alga/.venv/bin/alga). See the shim source for
# the opt-out env var (ALGA_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/alga-exec-shim.sh /opt/alga/bin/alga

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> alga ...` and any
# subprocess that doesn't activate the venv first still find alga.
#
# /opt/alga/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/alga/bin:/opt/alga/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. alga
# subcommand vs. no-args), drops to the alga user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/alga/docker/main-wrapper.sh" ]
CMD [ ]
