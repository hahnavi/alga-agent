...
There are two ways to add a platform to the Alga gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.alga/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Alga code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `alga gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `alga_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Alga core.
...
And add it to the `alga-gateway` composite:

```python
"alga-gateway": {
    "includes": [..., "alga-your-platform"]
}
```
...
## 12. Status Display (`alga_cli/status.py`)
...
## 13. Gateway Setup Wizard (`alga_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ alga_cli/ toolsets.py \
  --include="*.py" -l | sort -u
