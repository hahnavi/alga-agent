    ...
    def __getattr__(cls, name):
        try:
            ...
            try:
                ...
                raise AttributeError(f"Platform has no attribute {name}")


REMOVED_PLATFORM_VALUES = {
    # Built-in platforms on main:
    "discord", "whatsapp", "slack", "signal", "mattermost", "matrix",
    ...
    # Bundled plugin platforms on main:
    "google_chat", "irc", "line", "ntfy", "simplex", "teams"
}


class Platform(Enum, metaclass=DynamicEnumMeta):
    ...
    def _missing_(cls, value):
        """Accept unknown platform names dynamically.
    ...
    def is_known(cls, value: str) -> bool:
        """Check if a platform name represents a built-in, removed, or registered platform."""
        if not isinstance(value, str):
            return False
        val = value.strip().lower()
        if val in {"local", "telegram"}:
            return True
        if val in REMOVED_PLATFORM_VALUES:
            return True
        try:
            from gateway.platform_registry import platform_registry
            if platform_registry.is_registered(val):
                return True
        except Exception:
            pass
        return False
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_alga_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.alga/config.yaml (primary user-facing config)
    3. ~/.alga/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_alga_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``alga config set gateway.streaming.*``
            ...
            for plat in _shared_loop_targets:
                ...
                has_platform_keys = any(k in platform_cfg for k in ("token", "api_key", "home_channel", "reply_to_mode", "gateway_restart_notification"))
                if not bridged and not enabled_was_explicit and not has_platform_keys:
                    continue
                ...
                if "extra" in platform_cfg and isinstance(platform_cfg["extra"], dict):
                    extra.update(platform_cfg["extra"])
