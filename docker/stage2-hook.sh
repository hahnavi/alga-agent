...
#
# Wired into the image as /etc/cont-init.d/01-alga-setup by the
...
ALGA_HOME="${ALGA_HOME:-/opt/data}"
INSTALL_DIR="/opt/alga"

# Drop to alga via s6-setuidgid, but skip it when already non-root.
as_alga() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid alga "$@"; }

# --- Bootstrap ALGA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid alga mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ALGA_HOME paths whose parents only
# root can create (e.g. `ALGA_HOME=/home/alga/.alga` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ALGA_UID/ALGA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ALGA_UID/
# ALGA_GID still win when both are set.  See #15290, salvages #25872.
ALGA_UID="${ALGA_UID:-${PUID:-}}"
ALGA_GID="${ALGA_GID:-${PGID:-}}"

if [ -n "${ALGA_UID:-}" ] && validate_uid_gid "$ALGA_UID" && [ "$ALGA_UID" != "$(id -u alga)" ]; then
    echo "[stage2] Changing alga UID to $ALGA_UID"
    usermod -u "$ALGA_UID" alga
fi
if [ -n "${ALGA_GID:-}" ] && validate_uid_gid "$ALGA_GID" && [ "$ALGA_GID" != "$(id -g alga)" ]; then
    echo "[stage2] Changing alga GID to $ALGA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ALGA_GID" alga 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised alga user (UID 10000)
...
# alga process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes alga. Idempotent across container
...
#   - alga is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G alga 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] alga already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" alga 2>/dev/null; then
        echo "[stage2] Added alga to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group alga failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ALGA_UID is remapped or the top-level $ALGA_HOME isn't owned by
# the runtime alga UID, restore ownership to alga — but ONLY for the
# directories alga actually writes to. The full $ALGA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of alga-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_alga_uid=$(id -u alga)
needs_chown=false
if [ "$(stat -c %u "$ALGA_HOME" 2>/dev/null)" != "$actual_alga_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ALGA_HOME (targeted) to alga ($actual_alga_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ALGA_HOME: chown the directory itself (not its contents)
    # so alga can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown alga:alga "$ALGA_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $ALGA_HOME failed (rootless container?) — continuing"
    # Alga-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by alga (see the s6-setuidgid mkdir
    # -p block below for the canonical list).
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles; do
        if [ -e "$ALGA_HOME/$sub" ]; then
            chown -R alga:alga "$ALGA_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ALGA_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
    # Alga-owned trees under $INSTALL_DIR must be re-chowned when the UID
    ...
    #     the source mtime is newer than dist/ or when ALGA_TUI_FORCE_BUILD
    #     is set) and writes to ui-tui/dist/. Without this chown the new
    #     alga UID can't write the build output (#28851).
    #   - node_modules: root-level dependencies (puppeteer, web tooling)
    #     that runtime code may walk/update.
    # The set mirrors the build-time `chown -R alga:alga` line in the
    # Dockerfile — keep them in sync if the Dockerfile chown set changes.
    # These are under $INSTALL_DIR (not $ALGA_HOME), so the bind-mount
...
# Always reset ownership of $ALGA_HOME/profiles to alga on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> alga …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as alga and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ALGA_HOME/profiles" ]; then
    chown -R alga:alga "$ALGA_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of alga-owned top-level state files on every boot.
# The targeted data-volume chown above only covers alga-owned
# *subdirectories*; loose state files living directly under $ALGA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> alga …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged alga runtime then hits
...
# sweep so host-owned files in a bind-mounted $ALGA_HOME are never
...
# entries of alga_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ALGA_HOME/$f" ]; then
        chown alga:alga "$ALGA_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the alga runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ALGA_HOME/config.yaml" ]; then
    chown alga:alga "$ALGA_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$ALGA_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as alga user ---
# Run as alga via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ALGA_HOME values
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ALGA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_alga cp "$INSTALL_DIR/$src" "$ALGA_HOME/$dest"
    fi
}
...
if [ -f "$ALGA_HOME/.env" ]; then
    chown alga:alga "$ALGA_HOME/.env" 2>/dev/null || true
    chmod 600 "$ALGA_HOME/.env" 2>/dev/null || true
fi

# auth.json: bootstrap from env on first boot only. Same semantics as the
# pre-s6 entrypoint — the [ ! -f ] guard is critical to avoid clobbering
# rotated refresh tokens on container restart.
if [ ! -f "$ALGA_HOME/auth.json" ] && [ -n "${ALGA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$ALGA_AUTH_JSON_BOOTSTRAP" > "$ALGA_HOME/auth.json"
    chown alga:alga "$ALGA_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$ALGA_HOME/auth.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_alga "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/alga/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Alga spawns for the
...
# main-wrapper.sh propagates it into the supervised ``alga`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-alga, etc.).
