"""Persistent slash-command worker — one AlgaCLI per TUI session.
...
def _run(cli: AlgaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ALGA_SESSION_KEY"] = args.session_key
    os.environ["ALGA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AlgaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
