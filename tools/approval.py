...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ALGA_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ALGA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("ALGA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ALGA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ALGA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ALGA_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ALGA_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $ALGA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ALGA_ENV_PATH = (
    r'(?:~\/\.alga/|'
    r'(?:\$home|\$\{home\})/\.alga/|'
    r'(?:\$alga_home|\$\{alga_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\balga\s+gateway\s+(stop|restart)\b', "stop/restart alga gateway (kills running agents)"),
    (r'\balga\s+update\b', "alga update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `alga gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart alga-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart alga-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(alga|gateway|cli\.py)\b', "kill alga/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill alga` but not
    # `kill -9 $(pgrep -f alga)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ALGA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ALGA_INTERACTIVE or ALGA_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ALGA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ALGA_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ALGA_EXEC_ASK")
