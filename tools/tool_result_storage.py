...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/alga-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/alga-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/alga-results"
HEREDOC_MARKER = "ALGA_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/alga-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ALGA_PERSIST_{uuid.uuid4().hex[:8]}"
