...
it is automatically downloaded from GitHub releases to $ALGA_HOME/bin/tirith.
...
def _get_alga_home() -> str:
    """Return the Alga home directory, respecting ALGA_HOME env var."""
    return str(get_alga_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_alga_home(), ".tirith-install-failed")
...
def _alga_bin_dir() -> str:
    """Return $ALGA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_alga_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ALGA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_alga_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ALGA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ALGA_HOME/bin/tirith
    ...
    alga_bin = os.path.join(_alga_bin_dir(), "tirith")
    if os.path.isfile(alga_bin) and os.access(alga_bin, os.X_OK):
        _resolved_path = alga_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return alga_bin
