...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``alga tools`` repaint, tool-registration scans,
    ...
    2. ``~/.alga/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_alga_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.alga/.env`` first, then ``os.environ``.

    Wraps :func:`alga_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from alga_cli.config import get_env_value as _alga_get_env_value

        value = _alga_get_env_value(name)
...
def alga_xai_user_agent() -> str:
    """Return a stable Alga-specific User-Agent for xAI HTTP calls."""
    try:
        from alga_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Alga-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Alga-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``alga_cli.config.get_env_value`` so keys
    stored in ``~/.alga/.env`` (the standard Alga location) are honored —
