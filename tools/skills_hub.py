#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Alga Skills Hub.
...
Used by alga_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            alga_meta = metadata.get("alga", {})
            if isinstance(alga_meta, dict):
                tags = alga_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.alga/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# Alga centralized index source
# ---------------------------------------------------------------------------

ALGA_INDEX_URL = "https://alga-agent.hahnavi.my.id/docs/api/skills-index.json"
ALGA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "alga-index.json"
ALGA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_alga_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ALGA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ALGA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ALGA_INDEX_CACHE_FILE.stat().st_mtime
            if age < ALGA_INDEX_TTL:
                return json.loads(ALGA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ALGA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Alga index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Alga index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ALGA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ALGA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class AlgaIndexSource(SkillSource):
    """Skill source backed by the centralized Alga Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "alga-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "alga-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AlgaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
