...
This module provides the glue:
    - ``AlgaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Alga Agent"
"""
...
from alga_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__alga_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ALGA_HOME so each profile gets its own OAuth tokens.
    Layout: ``ALGA_HOME/mcp-tokens/``
    """
    try:
        from alga_constants import get_alga_home
        base = Path(get_alga_home())
    except ImportError:
        base = Path(os.environ.get("ALGA_HOME", str(Path.home() / ".alga")))
...
# ---------------------------------------------------------------------------
# AlgaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AlgaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ALGA_HOME/mcp-tokens/<server_name>.json         -- tokens
        ALGA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ALGA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Alga records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Alga.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://alga-agent.hahnavi.my.id/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `alga mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = AlgaTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Alga Agent")
    scope = cfg.get("scope")
