...
`mcp_tool.py`, and `alga_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AlgaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_alga_provider_class() -> Optional[type]:
    ...
    class AlgaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._alga_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AlgaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AlgaTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._alga_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AlgaTokenStorage
                        if isinstance(storage, AlgaTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._alga_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AlgaTokenStorage
            if not isinstance(storage, AlgaTokenStorage):
                return
    ...
    return AlgaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ALGA_PROVIDER_CLS: Optional[type] = _make_alga_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AlgaMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``alga mcp remove <name>`` and (indirectly) by
        ``alga mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
