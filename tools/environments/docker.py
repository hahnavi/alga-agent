...
from tools.environments.local import _ALGA_PROVIDER_ENV_BLOCKLIST
...
def _load_alga_env_vars() -> dict[str, str]:
    """Load ~/.alga/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Alga profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale alga-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=alga-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Alga process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=alga-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a alga process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Alga
    process leaves its container behind permanently because there's no
    subsequent Alga process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=alga-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=alga-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ALGA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ALGA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ALGA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'alga'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``alga-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.alga/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. alga-agent:latest) already use /init as PID 1
        ...
        container_name = f"alga-{uuid.uuid4().hex[:8]}"
        # Labels make alga-created containers identifiable to:
        #   * the orphan reaper (`alga-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`alga-task-id`, `alga-profile`)
        #   * operators running `docker ps --filter label=alga-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Alga profile is captured at
        ...
        label_args = [
            "--label", "alga-agent=1",
            "--label", f"alga-task-id={task_label}",
            "--label", f"alga-profile={profile_name}",
        ]
        self._labels = {
            "alga-agent": "1",
            "alga-task-id": task_label,
            "alga-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Alga process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Alga secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ALGA_PROVIDER_ENV_BLOCKLIST)
        alga_env = _load_alga_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = alga_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``alga-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=alga-agent=1",
                    "--filter", f"label=alga-task-id={task_label}",
                    "--filter", f"label=alga-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Alga process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Alga exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Alga process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Alga startup. That covers the
        ...
        # outlives Alga processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"alga-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``alga /quit``
