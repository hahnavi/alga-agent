...
def _is_alga_provider_credential(name: str) -> bool:
    """True if ``name`` is a Alga-managed provider credential (API key,
    token, or similar) per ``_ALGA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Alga API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _ALGA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Alga-managed provider credentials (from
    ``_ALGA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Alga-managed
    ...
    Non-Alga third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_alga_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Alga provider "
                "credential %r (blocked by _ALGA_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Alga-managed provider credentials must not be passed
                ...
                if _is_alga_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Alga "
                        "provider credential %r from config.yaml (blocked "
                        "by _ALGA_PROVIDER_ENV_BLOCKLIST). Operator "
