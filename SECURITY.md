# Alga Agent Security Policy

This document describes Alga Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/hahnavi/alga-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Alga Agent does not operate a bug
bounty program.**
...
- Environment details (`alga version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Alga Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Alga Agent's documentation
...
Alga Agent supports two OS-level isolation postures. They address
...
Alga Agent supports this in two ways:

- **Alga Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Alga Agent's in-process heuristics
...
Alga Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Alga Agent
itself. Bugs in Alga Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Alga Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Alga Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Alga Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  alga-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Alga Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Alga Agent. Bugs in Alga
...
- Alga Agent includes supply-chain guards for MCP server
