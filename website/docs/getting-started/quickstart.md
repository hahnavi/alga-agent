...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Alga Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Alga setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Alga Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Alga Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Alga for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Alga working on my machine | `alga setup` | Run a real chat and verify it responds |
| I already know my provider | `alga model` | Save the config, then start chatting |
| I want a bot or always-on setup | `alga gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `alga model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `alga model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Alga cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Alga Agent

**Option A — pip (simplest):**

```bash
pip install alga-agent
alga postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
- **Desktop downloads:** [GitHub Releases](https://github.com/hahnavi/alga-agent/releases/latest)
...
The single most important setup step. Use `alga model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `alga model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `alga model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `alga model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `alga_cli/models.py` may change between releases) | `alga model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `alga model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `alga model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `alga model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `alga model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `alga model` (requires `copilot` CLI + `copilot login`) |
...
Alga Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `alga model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Alga separates secrets from normal config:

- **Secrets and tokens** → `~/.alga/.env`
- **Non-secret settings** → `~/.alga/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
alga config set model anthropic/claude-opus-4.6
alga config set terminal.backend docker
alga config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
alga            # classic CLI
alga --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Alga ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `alga` vs `alga --tui`.
:::
...
```bash
alga --continue    # Resume the most recent session
alga -c            # Short form
```
...
```bash
alga gateway setup    # Interactive platform configuration
```
...
- `alga tools` — tune tool access per platform
- `alga skills` — browse and install reusable workflows
...
```bash
alga config set terminal.backend docker    # Docker isolation
alga config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Alga install directory (the curl installer placed it at
# ~/.alga/alga-agent on Linux/macOS or %LOCALAPPDATA%\alga\alga-agent on Windows):
cd ~/.alga/alga-agent
...
(If you installed without `[all]`, run `cd ~/.alga/alga-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Alga opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `alga model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `alga gateway setup` and check `alga gateway status` |
| `alga --continue` can't find old session | Switched profiles or session never saved | Check `alga sessions list` and confirm you're in the right profile |
...
| `alga doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `alga model`
3. `alga setup`
4. `alga sessions list`
5. `alga --continue`
6. `alga gateway status`
...
| `alga tools` | Configure which tools are enabled per platform |
| `alga setup` | Full setup wizard (configures everything at once) |
| `alga doctor` | Diagnose issues |
| `alga update` | Update to latest version |
| `alga gateway` | Start the messaging gateway |
| `alga --continue` | Resume last session |
