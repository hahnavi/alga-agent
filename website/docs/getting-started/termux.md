...
description: "Run Alga Agent directly on an Android phone with Termux"
---

# Alga on Android with Termux

This is the tested path for running Alga Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Alga from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Alga now ships a Termux-aware installer path:
...
- links `alga` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `alga` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/alga" "$PREFIX/bin/alga"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `alga` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.alga/.env`.
...
### `alga doctor` says ripgrep or Node is missing
