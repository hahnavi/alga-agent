...
Connect Alga to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Alga supports two WeCom integration modes:
...
> Run `alga gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Alga decrypts the message, queues it for the agent
...
(Use `alga gateway start` only after `alga gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Alga and the token the
...
in `~/.alga/.env` values around `=` will also break signature checks. After
fixing, restart `alga gateway run`.
...
Check `alga gateway run` logs for the bound host/port. If the adapter bound to
