...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Alga Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Alga to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.alga/.env`:
...
All standard Alga commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.alga/logs/gateway.log`
...
4. Consider enabling verbose logging: `ALGA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Alga automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ALGA_LOG_LEVEL=debug alga gateway
```
...
```bash
alga chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
