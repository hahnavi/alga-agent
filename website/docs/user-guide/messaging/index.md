...
description: "Chat with Alga from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Alga from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Alga](/guides/use-voice-mode-with-alga).
...
```bash
alga gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
alga gateway              # Run in foreground
alga gateway setup        # Configure messaging platforms interactively
alga gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo alga gateway install --system   # Linux only: install a boot-time system service
alga gateway start        # Start the default service
alga gateway stop         # Stop the default service
alga gateway status       # Check default service status
alga gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Alga Agent to the latest version |
...
Configure per-platform overrides in `~/.alga/gateway.json`:
...
alga pairing approve telegram XKGH5N7P

# Other pairing commands:
alga pairing list          # View pending + approved users
alga pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Alga appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.alga/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.alga/config.yaml`:
...
```bash
ALGA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
alga gateway install               # Install as user service
alga gateway start                 # Start the service
alga gateway stop                  # Stop the service
alga gateway status                # Check status
journalctl --user -u alga-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo alga gateway install --system
sudo alga gateway start --system
sudo alga gateway status --system
journalctl -u alga-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Alga will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Alga installations on the same machine (with different `ALGA_HOME` directories), each gets its own systemd service name. The default `~/.alga` uses `alga-gateway`; other installations use `alga-gateway-<hash>`. The `alga gateway` commands automatically target the correct service for your current `ALGA_HOME`.
:::

### macOS (launchd)

```bash
alga gateway install               # Install as launchd agent
alga gateway start                 # Start the service
alga gateway stop                  # Stop the service
alga gateway status                # Check status
tail -f ~/.alga/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.alga.gateway.plist`. It includes three environment variables:
...
- **ALGA_HOME** — scopes the gateway to your Alga installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `alga gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ALGA_HOME` directory gets its own launchd label. The default `~/.alga` uses `ai.alga.gateway`; other installations use `ai.alga.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `alga-cli` | Full access |
| Telegram | `alga-telegram` | Full tools including terminal |
| Discord | `alga-discord` | Full tools including terminal |
| WhatsApp | `alga-whatsapp` | Full tools including terminal |
| Slack | `alga-slack` | Full tools including terminal |
| Google Chat | `alga-google_chat` | Full tools including terminal |
| Signal | `alga-signal` | Full tools including terminal |
| SMS | `alga-sms` | Full tools including terminal |
| Email | `alga-email` | Full tools including terminal |
| Home Assistant | `alga-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `alga-mattermost` | Full tools including terminal |
| Matrix | `alga-matrix` | Full tools including terminal |
| DingTalk | `alga-dingtalk` | Full tools including terminal |
| Feishu/Lark | `alga-feishu` | Full tools including terminal |
| WeCom | `alga-wecom` | Full tools including terminal |
| WeCom Callback | `alga-wecom-callback` | Full tools including terminal |
| Weixin | `alga-weixin` | Full tools including terminal |
| BlueBubbles | `alga-bluebubbles` | Full tools including terminal |
| QQBot | `alga-qqbot` | Full tools including terminal |
| Yuanbao | `alga-yuanbao` | Full tools including terminal |
| Microsoft Teams | `alga-teams` | Full tools including terminal |
| API Server | `alga-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `alga-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.alga/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
