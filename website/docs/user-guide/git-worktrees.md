...
title: "Git Worktrees"
description: "Run multiple Alga agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Alga Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Alga so each session has a clean, isolated working directory.

## Why Use Worktrees with Alga?

Alga treats the **current working directory** as the project root:

- CLI: the directory where you run `alga` or `alga chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.alga/config.yaml`
...
git worktree add ../repo-feature feature/alga-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/alga-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Alga there:

```bash
cd ../repo-feature

# Start Alga in the worktree
alga
```
...
git worktree add ../repo-experiment-a feature/alga-a
git worktree add ../repo-experiment-b feature/alga-b
```
...
- Works on its own branch (`feature/alga-a` vs `feature/alga-b`).
...
- Alga checkpoint data under `~/.alga/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Alga experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/alga-checkpoints-docs`, `feature/alga-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Alga from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `alga -w` (Automatic Worktree Mode)

Alga has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `alga/alga-<hash>`).
...
```bash
alga -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `alga -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Alga session its own clean checkout.
