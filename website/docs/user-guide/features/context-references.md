...
Type `@` followed by a reference to inject content directly into your message. Alga expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Alga env: `$ALGA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ALGA_HOME/skills/.hub/`
