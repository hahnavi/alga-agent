...
Alga Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `alga computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `alga tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.alga/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Alga refreshes the binary in two
places so you don't get stuck on a stale release:

- **`alga update`** — when you update Alga itself, if `cua-driver` is
...
- **`alga computer-use install --upgrade`** — manual force-refresh.
...
`alga computer-use status` shows the installed version next to the
binary path.
...
Alga applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.alga/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Alga applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ALGA_CUA_DRIVER_VERSION`
...
`alga computer-use install` to fetch the cua-driver binary, or run
`alga tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/hahnavi/alga-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
