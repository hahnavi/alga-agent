...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Alga's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Alga instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
alga memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `alga` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Alga syncs them back at session init.
...
The `alga honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `alga memory setup honcho` (or run `alga memory setup` and pick it from the list); the `alga honcho` subcommand then appears on the next invocation.

```bash
alga memory setup honcho    # Configure Honcho directly (works before activation)
alga honcho status          # Connection status, config, and key settings
alga honcho setup           # Redirects to `alga memory setup` (post-activation alias)
alga honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
alga honcho peer            # Show or update peer names + dialectic reasoning level
alga honcho mode            # Show or set recall mode (hybrid/context/tools)
alga honcho tokens          # Show or set token budget for context and dialectic
alga honcho identity        # Seed or show the AI peer's Honcho identity
alga honcho sync            # Sync Honcho config to all existing profiles
alga honcho peers           # Show peer identities across all profiles
alga honcho sessions        # List known Honcho session mappings
alga honcho map             # Map current directory to a Honcho session name
alga honcho enable          # Enable Honcho for the active profile
alga honcho disable         # Disable Honcho for the active profile
alga honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `alga honcho`

If you previously used the standalone `alga honcho setup`:
...
No re-login or re-setup needed. Run `alga memory setup` and select "honcho" — the wizard detects your existing config.
