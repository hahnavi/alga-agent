...
Alga Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
alga memory setup      # interactive picker + configuration
alga memory status     # check what's active
alga memory off        # disable external provider
```

You can also select the active memory provider via `alga plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.alga/config.yaml`:
...
When a memory provider is active, Alga automatically:
...
**Setup Wizard:**
```bash
alga memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

On a fresh install, configure Honcho directly with `alga memory setup honcho`. The legacy `alga honcho setup` command still works (it now redirects to `alga memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$ALGA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ALGA_HOME/honcho.json` > `~/.alga/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/hahnavi/alga-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/alga).
...
:::tip Migrating from `alga honcho`
If you previously used `alga honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Alga profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Alga profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Alga profile. Host key `alga` → default; `alga.<profile>` for others. |
...
```bash
alga profile create coder --clone
```

`--clone` creates a `alga.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Alga profile, creates host blocks for any profile without one, inherits settings from the default `alga` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/hahnavi/alga-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/alga).
...
# Then configure Alga
alga memory setup    # select "openviking"
# Or manually:
alga config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.alga/.env
```
...
**Config:** `$ALGA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `alga-user` | User identifier |
...
**Setup:**
```bash
alga memory setup    # select "hindsight"
# Or manually:
alga config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.alga/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p alga ui start`

**Config:** `$ALGA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `alga` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Alga Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/hahnavi/alga-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
alga memory setup    # select "holographic"
# Or manually:
alga config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.alga-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ALGA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
alga memory setup    # select "retaindb"
# Or manually:
alga config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.alga/.env
```
...
# Then configure Alga
alga memory setup    # select "byterover"
# Or manually:
alga config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ALGA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
alga memory setup    # select "supermemory"
# Or manually:
alga config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.alga/.env
```

**Config:** `$ALGA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `alga` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `alga-{identity}` → `alga-coder`) to isolate memories per Alga profile
...
| **Requires** | `pip install alga-memori` + `alga-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install alga-memori
alga-memori install
alga config set memory.provider memori
alga memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `alga-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ALGA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ALGA_HOME/` so each profile has its own credentials
