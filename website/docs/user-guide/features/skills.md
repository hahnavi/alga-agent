...
All skills live in **`~/.alga/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Alga at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Alga to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.alga/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
alga chat --toolsets skills -q "What skills do you have?"
alga chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.alga/cache/chart-q4-2025.png
...
When a missing value is encountered, Alga asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `alga setup` or `~/.alga/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `alga config migrate` prompts for unconfigured settings, and `alga config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.alga/skills/                  # Single source of truth
...
If you maintain skills outside of Alga — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Alga to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.alga/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.alga/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Alga process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Alga ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.alga/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.alga/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `alga bundles list`.
...
alga bundles create research

# Overwrite an existing bundle
alga bundles create backend-dev --skill ... --force

# Delete a bundle
alga bundles delete backend-dev

# Re-scan ~/.alga/skill-bundles/ and report changes
alga bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.alga/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.alga/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
alga skills browse                              # Browse all hub skills (official first)
alga skills browse --source official            # Browse only official optional skills
alga skills search kubernetes                   # Search all sources
alga skills search react --source skills-sh     # Search the skills.sh directory
alga skills search https://mintlify.com/docs --source well-known
alga skills inspect openai/skills/k8s           # Preview before installing
alga skills install openai/skills/k8s           # Install with security scan
alga skills install official/security/1password
alga skills install skills-sh/vercel-labs/json-render/json-render-react --force
alga skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
alga skills list --source hub                   # List hub-installed skills
alga skills check                               # Check installed hub skills for upstream updates
alga skills update                              # Reinstall hub skills with upstream changes when needed
alga skills audit                               # Re-scan all hub skills for security
alga skills uninstall k8s                       # Remove a hub skill
alga skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
alga skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
alga skills publish skills/my-skill --to github --repo owner/repo
alga skills snapshot export setup.json          # Export skill config
alga skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Alga. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `alga skills search <query> --source skills-sh`. Alga resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Alga currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Alga repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Alga can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
alga skills search https://mintlify.com/docs --source well-known
alga skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
alga skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Alga can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
alga skills install openai/skills/k8s
alga skills tap add myorg/skills-repo
```
...
[Skills Hub](https://alga-agent.hahnavi.my.id/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Alga-side changes required.
...
- Alga source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Alga supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Alga source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Alga can search and convert agent entries from LobeHub's public catalog into installable Alga skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Alga source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Alga integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Alga' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Alga source id: `browse-sh`
- Trust level: `community`

```bash
alga skills search airbnb --source browse-sh
alga skills inspect browse-sh/airbnb.com/search-listings-ddgioa
alga skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Alga fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
alga skills install https://sharethis.chat/SKILL.md
alga skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
alga skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `alga skills update` re-fetches from the same URL automatically when you want to refresh.
...
`alga skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Alga | Always trusted |
...
```bash
alga skills check          # Report which installed hub skills changed upstream
alga skills update         # Reinstall only the skills with updates available
alga skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Alga users add with `alga skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.alga.tags`, `version`, `author`, `platforms`, `metadata.alga.config`).
...
Alga discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Alga user can subscribe and install:

```bash
alga skills tap add my-org/alga-skills
alga skills search deploy
alga skills install my-org/alga-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.alga/.hub/taps.json`:
...
The `alga skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `alga skills tap list` shows the effective path per tap.
...
```bash
alga skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.alga/.hub/taps.json` (created on demand).

## Bundled skill updates (`alga skills reset`)

Alga ships with a set of bundled skills in `skills/` inside the repo. On install and on every `alga update`, a sync pass copies those into `~/.alga/skills/` and records a manifest at `~/.alga/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Alga recomputes the hash of your local copy and compares it to the origin hash:
...
alga skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `ALGA_HOME`, so `alga -p coder skills reset <name>` only affects that profile.
:::
