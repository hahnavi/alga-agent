...
description: "Plugins shipped with Alga Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Alga ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.alga/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Alga Plugin](/guides/build-a-alga-plugin) to write your own.
...
2. **User** — `~/.alga/plugins/<name>/`
3. **Project** — `./.alga/plugins/<name>/` (requires `ALGA_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `alga_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `alga memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `alga plugins list` and the interactive `alga plugins` UI), but none load until you explicitly enable them:

```bash
alga plugins enable disk-cleanup
```

Or via `~/.alga/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Alga. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
alga plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `alga plugins enable <name>`.
...
| `alga-achievements` | dashboard tab | Steam-style collectible badges generated from your real Alga session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `alga memory` and `alga plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALGA_HOME` or `/tmp/alga-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ALGA_HOME | always | Never |
...
**State** — everything lives at `$ALGA_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ALGA_HOME` or `/tmp/alga-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `alga plugins enable disk-cleanup` (or check the box in `alga plugins`).

**Disabling again:** `alga plugins disable disk-cleanup`.
...
**Enabling:** `alga plugins enable security-guidance` (or check the box in `alga plugins`).

**Disabling again:** `alga plugins disable security-guidance`.
...
Traces Alga turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Alga' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `alga logs`.
...
```bash
alga tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Alga and the next turn ships a trace.
...
Then put the credentials in `~/.alga/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Alga turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ALGA_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Alga session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `alga chat` session lives under one Langfuse session.

**Verify:**

```bash
alga plugins list                 # observability/langfuse should show "enabled"
alga chat -q "hello"              # check the Langfuse UI for a "Alga turn" trace
```
...
| `ALGA_LANGFUSE_RELEASE` | — | Release/version tag |
| `ALGA_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ALGA_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ALGA_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Alga-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Alga-prefixed wins when both are set.
...
**Disabling:** `alga plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.alga/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
alga plugins enable google_meet
...
**Disabling:** `alga plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.alga/cache/google_meet/` until you remove them.

### alga-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Alga session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Alga feature changes.

**How it works:**

- Scans your entire `~/.alga/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ALGA_HOME/plugins/alga-achievements/state.json`
...
| Secret | Hidden until Alga detects the first related signal in your history |

**API** — routes mount under `/api/plugins/alga-achievements/`:
...
**State files** — live under `$ALGA_HOME/plugins/alga-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Alga updates. |
...
**Enabling:** Nothing to enable — `alga-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `alga dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/alga-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.alga/plugins/alga-achievements/` that ships no dashboard. The plugin's state files under `$ALGA_HOME/plugins/alga-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Alga plugin — see [Build a Alga Plugin](/guides/build-a-alga-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.alga/plugins/<name>/`
- Manifest source is reported as `bundled` in `alga plugins list`
