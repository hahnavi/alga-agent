...
Alga runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Alga
...
1. Alga captures a baseline of current diagnostics for the file.
...
…). Alga auto-detects the binary on PATH or in
`<ALGA_HOME>/lsp/bin/`.
...
`node_modules` tree — Alga installs both packages together when you
run `alga lsp install typescript` or auto-install fires on first
use.

## CLI

```
alga lsp status          # service state + per-server install status
alga lsp list            # registry, optionally --installed-only
alga lsp install <id>    # eagerly install one server
alga lsp install-all     # try every server with a known recipe
alga lsp restart         # tear down running clients
alga lsp which <id>      # print resolved binary path
```

`alga lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ALGA_HOME>/lsp/bin
...
When `install_strategy: auto`, Alga installs binaries into
`<ALGA_HOME>/lsp/bin/`. NPM packages land in
`<ALGA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Alga-owned and is
...
Servers are kept alive for the life of the Alga process. There's
...
**`alga lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ALGA_HOME>/lsp/bin/`. Run
`alga lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `alga lsp status`**
...
When `alga lsp status` shows a `Backend warnings` section, install
...
Check `~/.alga/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `alga lsp restart` to clear the set;
