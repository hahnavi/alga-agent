...
chat completions — use your Alga-managed provider subscription as their
...
| Use case | "Use Alga as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Alga stores
the refresh token in `~/.alga/auth.json` — the same place all Alga
...
interface in `alga_cli/proxy/adapters/`.
...
If you see `not logged in`, run `alga auth add nous`. If you see
...
`alga_cli/proxy/adapters/<provider>.py` and registering it in
