...
If the file is empty, whitespace-only, or cannot be read, Alga falls back to a built-in default identity ("You are Alga Agent, an intelligent AI assistant created by Alga..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
