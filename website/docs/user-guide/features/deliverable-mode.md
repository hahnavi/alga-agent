...
When Alga Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.alga/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.alga/config.yaml` (switchable per session
via `/personality`).
...
If you use Alga' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.alga/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Alga Agent's deliverable
