...
- Give it a name like "alga-agent"
- Select scopes:
...
- Give it a title like "alga-agent-&lt;machine-name>"
...
elif [ -f ~/.alga/.env ] && grep -q "^GITHUB_TOKEN=" ~/.alga/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.alga/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
