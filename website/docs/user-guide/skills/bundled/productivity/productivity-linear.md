...
2. Set `LINEAR_API_KEY` in your environment (via `alga setup` or your env config)
...
```bash
SCRIPT=$(dirname "$(find ~/.alga -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
...
The trailing hex segment is the `slugId`. Example: `https://linear.app/Alga/document/rfc-hermes-permission-gateway-discord-38359beef67c` → `slugId` is `38359beef67c`.
