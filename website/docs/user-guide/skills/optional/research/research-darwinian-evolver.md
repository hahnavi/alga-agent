...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Alga Agent |
| License | MIT |
...
import upstream classes into Alga itself.
...
```bash
mkdir -p ~/.alga/cache/darwinian-evolver && cd ~/.alga/cache/darwinian-evolver
...
```bash
cd ~/.alga/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.alga/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.alga/skills/research/darwinian-evolver
DE_DIR=~/.alga/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Alga core.
   Custom driver scripts under `~/.alga/skills/...` are user-side and fine.
