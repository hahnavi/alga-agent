...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Alga Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.alga/.env`:
...
```bash
CANVAS="python $ALGA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
