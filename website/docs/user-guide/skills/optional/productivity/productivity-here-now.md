...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ALGA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ALGA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ALGA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client alga
```
...
```bash
PUBLISH="${ALGA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client alga
```
...
```bash
DRIVE="${ALGA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `alga`)    |
