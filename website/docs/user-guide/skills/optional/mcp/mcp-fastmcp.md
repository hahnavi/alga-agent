...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Alga or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Alga. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.alga/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.alga/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Alga integration, either:

- configure the server in `~/.alga/config.yaml` using the `native-mcp` skill, or
...
### Alga cannot see the deployed server

The server-building part may be correct while the Alga config is not. Load the `native-mcp` skill and configure the server in `~/.alga/config.yaml`, then restart Alga.
