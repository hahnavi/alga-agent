...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/creative/meme-generation` |
...
4. Find the skill's script directory:
   ```
   SKILL_DIR=$(dirname "$(find ~/.alga/skills -path '*/meme-generation/SKILL.md' 2>/dev/null | head -1)")
   ```
5. Run the generator:
   ```bash
