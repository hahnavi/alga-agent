...
| | |
|---|---|
| Source | Optional — install with `alga skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Alga Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Alga terminal calls (desktop app flow only)
...
### Service Account (recommended for Alga)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.alga/.env` (the skill will prompt for this on first load).
...
## Alga Execution Pattern (desktop app flow)

Alga terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/alga-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/alga-op.sock"
