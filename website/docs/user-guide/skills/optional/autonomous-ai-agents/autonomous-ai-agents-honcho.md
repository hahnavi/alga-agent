...
description: "Configure and use Honcho memory with Alga -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Alga -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `alga skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Alga

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Alga profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/alga#running-honcho-locally-with-alga

### Verify

```bash
alga honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Alga profile's AI peer
...
Honcho models conversations as interactions between **peers**. Alga creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Alga instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Alga run |
| `global` | Single session across all directories |

Manual override: `alga honcho map my-project-name`
...
Each Alga profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block alga.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `alga.coder` host block in `honcho.json`
...
```bash
alga honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Alga when Honcho memory is active.
...
Config file: `$ALGA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `alga honcho setup`. Ensure `memory.provider: honcho` is in `~/.alga/config.yaml`.

### Memory not persisting across sessions
Check `alga honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `alga profile create <name> --clone`. For existing profiles: `alga honcho sync`.
...
| Command | Description |
|---------|-------------|
| `alga honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `alga honcho status` | Show resolved config, connection test, peer info for active profile |
| `alga honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `alga honcho disable` | Disable Honcho for the active profile |
| `alga honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `alga honcho peers` | Show peer identities across all profiles |
| `alga honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `alga honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `alga honcho sessions` | List known directory-to-session-name mappings |
| `alga honcho map <name>` | Map current working directory to a Honcho session name |
| `alga honcho identity` | Seed AI peer identity or show both peer representations |
| `alga honcho sync` | Create host blocks for all Alga profiles that don't have one yet |
| `alga honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Alga + Honcho |
| `alga memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `alga memory status` | Show active memory provider and config |
| `alga memory off` | Disable external memory provider |
