---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Alga Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Alga Agent"
---
...
Migrate a user's OpenClaw customization footprint into Alga Agent. Imports Alga-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Related skills | [`alga-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-alga-agent) |
...
# OpenClaw -> Alga Migration

Use this skill when a user wants to move their OpenClaw setup into Alga Agent with minimal manual cleanup.
...
```bash
alga claw migrate              # Full interactive migration
...
alga claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `alga setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_alga.py` to:

- import `SOUL.md` into the Alga home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Alga memory entries
- merge OpenClaw command approval patterns into Alga `command_allowlist`
- migrate Alga-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Alga working-directory configuration
- copy OpenClaw skills into `~/.alga/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Alga workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.alga/tts/`
- archive non-secret docs that do not have a direct Alga destination
...
- `scripts/openclaw_to_alga.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py`

Do not guess a shorter path like `~/.alga/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.alga/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Alga-compatible secrets, currently:
...
Alga CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Alga. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Alga workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Alga skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.alga/skills/migration/openclaw-migration/scripts/openclaw_to_alga.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Alga unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Alga targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Alga destination. Unsupported auth blobs must still be reported as skipped.
...
- Alga persona state imported
- Alga memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.alga/skills/openclaw-imports/`
