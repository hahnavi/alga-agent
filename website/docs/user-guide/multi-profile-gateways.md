...
If you only run one Alga agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Alga agents that should all
...
(`ai.alga.gateway-<name>.plist`) or systemd user service
(`alga-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
alga profile create coder
alga profile create personal-bot
alga profile create research
...
`~/.local/bin/alga-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: alga-gateways {start|stop|restart|status|list}"
}
...
```bash
alga-gateways start      # start every configured profile
alga-gateways stop       # stop every configured profile
alga-gateways restart    # restart all
alga-gateways status     # status across all
alga-gateways list       # delegates to `alga gateway list`
```

:::tip
The `default` profile is targeted with `alga gateway <action>` (no `-p`),
not `alga -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `alga -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.alga.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/alga-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.alga.gateway.plist` /
`alga-gateway.service`.
...
```bash
# Default profile
tail -f ~/.alga/logs/gateway.log
tail -f ~/.alga/logs/gateway.error.log

# Named profile
tail -f ~/.alga/profiles/<name>/logs/gateway.log
tail -f ~/.alga/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.alga/logs/gateway.log ~/.alga/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
alga logs --tail              # follow default profile
alga -p coder logs --tail     # follow one profile
alga logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
alga profile list             # profiles + model + gateway state
alga-gateways status          # full status across every profile
launchctl list | grep alga    # macOS — PIDs and labels
systemctl --user list-units 'alga-gateway-*'   # Linux — units
```
...
The default profile uses `~/.alga/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
alga config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.alga/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=alga --why="gateways running" \
  sleep infinity &
...
`alga-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.alga/.env ~/.alga/profiles/*/.env
```

## Updating the code

`alga update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `alga gateway start` after a previous `alga gateway stop`. The
...
```bash
ps -ef | grep "alga_cli.*-p <profile>"
cat ~/.alga/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.alga.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.alga.gateway-<profile>.plist

# Linux
systemctl --user restart alga-gateway-<profile>.service
```

### Health check

```bash
alga doctor                  # default profile
alga -p <profile> doctor     # one profile
```
