...
description: "Reference for Alga Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Alga may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Alga only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Alga does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Alga uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.alga/mcp-tokens/<server>.json` and reused across sessions
