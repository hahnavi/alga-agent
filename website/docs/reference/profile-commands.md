...
This page covers all commands related to [Alga profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `alga profile` without a subcommand shows help.
...
```bash
alga profile use <name>
```

Sets `<name>` as the active profile. All subsequent `alga` commands (without `-p`) will use this profile.
...
alga profile use default
```

## `alga profile create`

```bash
alga profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `alga profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `alga update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
alga profile create work2 --clone --clone-from work
```

## `alga profile describe`

```bash
alga profile describe [<name>] [options]
```
...
alga profile describe researcher

# Set it explicitly
alga profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
alga profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
alga profile describe --all --auto
```

## `alga profile delete`

```bash
alga profile delete <name> [options]
```
...
```bash
alga profile delete mybot
alga profile delete mybot --yes
```
...
```bash
alga profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Alga home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ alga profile show work
Profile: work
Path:    ~/.alga/profiles/work
...
```bash
alga profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Alga installation.
...
```bash
alga profile alias work
# Creates/updates ~/.local/bin/work

alga profile alias work --name mywork
# Creates ~/.local/bin/mywork

alga profile alias work --remove
...
```bash
alga profile rename <old-name> <new-name>
```
...
```bash
alga profile rename mybot assistant
# ~/.alga/profiles/mybot → ~/.alga/profiles/assistant
...
```bash
alga profile export <name> [options]
```
...
```bash
alga profile export work
# Creates work.tar.gz in the current directory

alga profile export work -o ./work-2026-03-29.tar.gz
```

## `alga profile import`

```bash
alga profile import <archive> [options]
```
...
```bash
alga profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

alga profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`alga profile export` / `import` are still the right commands for
...
### `alga profile install`

```bash
alga profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `alga -p telemetry`). |
...
alga profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
alga profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
alga profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
alga profile install ./telemetry/
```

### `alga profile update`

```bash
alga profile update <name> [--force-config] [--yes]
```
...
```bash
alga profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Alga version, author, env var requirements, the source URL/path, and
...
`alga profile list` also shows the distribution name and version in a
`Distribution` column, and `alga profile show <name>` / `delete <name>`
...
alga profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
alga profile install https://github.com/your-org/internal-assistant.git
```
...
`alga_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Alga version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Alga can clone from.
3. Tell recipients to run `alga profile install <your-repo-url>`.
...
```bash
alga -p <name> <command> [options]
alga --profile <name> <command> [options]
```

Global flag to run any Alga command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
alga -p work chat -q "Check the server status"
alga --profile dev gateway start
alga -p personal skills list
alga -p work config edit
```

## `alga completion`

```bash
alga completion <shell>
```
...
- `alga profile <TAB>` — subcommands (list, use, create, etc.)
- `alga profile use <TAB>` — profile names
- `alga -p <TAB>` — profile names
