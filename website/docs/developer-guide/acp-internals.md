...
The ACP adapter wraps Alga' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
alga acp / alga-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.alga/.env
  -> configure stderr logging
  -> construct AlgaACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'alga-agent[acp]==<version>' alga-acp`, pointed at the `alga-agent` PyPI release.
...
- `allow_once` -> Alga `once`
- `allow_always` -> Alga `always`
- reject options -> Alga `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Alga tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["alga-acp"])
...
Instead it reuses Alga' runtime resolver:

- `acp_adapter/auth.py`
- `alga_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Alga provider/credentials. It also always advertises a terminal setup auth method (`alga-setup`, args `--setup`) so first-run registry clients can open Alga' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.alga/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `alga-acp` toolset definition
- `alga_cli/main.py` — `alga acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `alga-acp` script
