...
Alga Agent uses a SQLite database (`~/.alga/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `alga_state.py`


## Architecture Overview

```
~/.alga/state.db (SQLite, WAL mode)
...
Multiple alga processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.alga/state.db
...
Default path: `~/.alga/state.db`

This is derived from `alga_constants.get_alga_home()` which resolves to
`~/.alga/` by default, or the value of `ALGA_HOME` environment variable.
