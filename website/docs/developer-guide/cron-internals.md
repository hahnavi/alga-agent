...
description: "How Alga stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `alga_cli/cron.py` | CLI `alga cron` subcommands |
...
Jobs are stored in `~/.alga/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `alga cron` commands are run or during active CLI sessions.
...
```python
# ~/.alga/scripts/check_competitors.py
...
2. **Environment variable** — `ALGA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.alga/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `alga cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `alga cron` CLI provides direct job management:

```bash
alga cron list                    # Show all jobs
alga cron create                  # Interactive job creation (alias: add)
alga cron edit <job_id>           # Edit job configuration
alga cron pause <job_id>          # Pause a running job
alga cron resume <job_id>         # Resume a paused job
alga cron run <job_id>            # Trigger immediate execution
alga cron remove <job_id>         # Delete a job
```
