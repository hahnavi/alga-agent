...
sidebar_position: 1
title: "Architecture"
description: "Alga Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Alga Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ alga_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AlgaCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── alga_state.py           # SQLite session/state database with FTS5
├── alga_constants.py       # ALGA_HOME, profile-aware paths
...
│
├── alga_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `alga` subcommands (large file)
...
│   ├── skills_config.py      # alga skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # alga tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # alga gateway start/stop
│
...
```text
User input → AlgaCLI.process_input()
...
Three discovery sources: `~/.alga/plugins/` (user), `.alga/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `alga plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-alga-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Alga as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`alga -p <name>`) gets its own ALGA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
