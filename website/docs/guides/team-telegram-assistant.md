...
This tutorial walks you through setting up a Telegram bot powered by Alga Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Alga Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.alga/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Alga itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Alga Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_alga_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Alga Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u alga-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `alga gateway install` to update the plist.
:::
...
```bash
alga gateway stop && alga gateway start
```
...
alga pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.alga/.env`:
...
Control how much detail the bot shows when using tools. In `~/.alga/config.yaml`:
...
Customize how the bot communicates by editing `~/.alga/SOUL.md`:

For a full guide, see [Use SOUL.md with Alga](/guides/use-soul-with-alga).
...
```bash
# From the CLI
alga cron list          # View all scheduled jobs
alga cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u alga-gateway` (Linux) or `~/.alga/logs/gateway.log` (macOS) |
| Cron job output | `~/.alga/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.alga/cron/jobs.json` |
| Pairing data | `~/.alga/pairing/` |
| Session history | `~/.alga/sessions/` |
