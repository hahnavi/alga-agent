...
3. Sets `model.provider: nous` in `~/.alga/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Alga is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
alga setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
alga auth add nous --type oauth --manual-paste
# Then re-run `alga setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Alga Agent release notes" and summarize the top 3 hits.
```

You should see Alga call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `alga setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
These rows appear in `alga tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Alga runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Alga will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
alga cron create "every day at 9am" \
...
If you use [Alga profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.alga/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `alga portal status` shows "not logged in" after `alga setup --portal`
...
```bash
alga config set model.provider nous
```
...
Re-verify with `alga portal status`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Alga doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/hahnavi/alga-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`alga portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Alga profiles where you're using the wrong one (check `alga profile current`)

### Want to revoke and start clean

```bash
alga auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Alga tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Alga configurations
