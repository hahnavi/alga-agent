...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.alga/.env` at the end.
...
4. Fill in:
   - **Name:** `Alga Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `alga-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.alga/.env`:
...
Alga ships a Graph auth smoke-test. From your Alga install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.alga/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `alga gateway restart`.
