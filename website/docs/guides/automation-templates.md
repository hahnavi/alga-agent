...
Copy-paste recipes for common automation patterns. Each template uses Alga's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`alga webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `alga webhook subscribe`) |
...
```bash
alga cron create "0 2 * * *" \
  "You are a project manager triaging the hahnavi/alga-agent GitHub repo.

1. Run: gh issue list --repo hahnavi/alga-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
alga webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
alga cron create "0 9 * * 1" \
  "Scan the hahnavi/alga-agent repo for documentation drift.

1. Run: gh pr list --repo hahnavi/alga-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (alga_cli/commands.py, alga_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (alga_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
alga cron create "0 6 * * *" \
  "Run a dependency security audit on the alga-agent project.

1. cd ~/.alga/alga-agent && source .venv/bin/activate
...
```bash
alga webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
alga webhook subscribe alert-triage \
...
```python title="~/.alga/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Alga-Monitor/1.0"})
...
alga cron create "every 30m" \
  ...
  --script ~/.alga/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
alga cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Alga Agent development." \
...
```bash
alga webhook subscribe github-issues \
  --events "issues" \
...
```bash
alga webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
alga webhook subscribe stripe-payments \
...
```bash
alga cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the alga-agent codebase.
