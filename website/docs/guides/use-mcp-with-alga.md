---
sidebar_position: 6
title: "Use MCP with Alga"
description: "A practical guide to connecting MCP servers to Alga Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Alga

This guide shows how to actually use MCP with Alga Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Alga tool
- you want Alga to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Alga to internal APIs, databases, or company systems without modifying Alga core

Do not use MCP when:
- a built-in Alga tool already solves the job well
...
- Alga remains the agent
- MCP servers contribute tools
- Alga discovers those tools at startup or reload time
...
If you installed Alga with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Alga banner/status should show MCP integration when configured
- ask Alga what tools it has available
...
## WSL2: bridge Alga in WSL to Windows Chrome

This is the practical setup when:

- Alga runs inside WSL2
...
In this setup, Alga does **not** connect to Chrome directly. Instead:

- Alga runs in WSL
- Alga starts a local stdio MCP server
...
```text
Alga (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Alga stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Alga core browser transport
...
```bash
alga mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
alga mcp test chrome-devtools-win
```

Then start a fresh Alga session or run:
...
Once loaded, Alga can use the MCP-prefixed browser tools directly. For example:
...
If Alga runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Alga from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Alga from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Alga:
...
So Alga will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Alga to reason over a bounded workspace.
...
Now Alga can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Alga core.
...
Because Alga now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
