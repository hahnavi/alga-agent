...
```bash
alga teams-pipeline token-health
alga teams-pipeline token-health --force-refresh
```
...
```bash
alga teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
alga teams-pipeline maintain-subscriptions
alga teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Alga cron (recommended if you already run the Alga gateway)

Alga ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.alga/scripts/`. First create the script:

```bash
mkdir -p ~/.alga/scripts
cat > ~/.alga/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec alga teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.alga/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
alga cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/alga-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Alga Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=alga
EnvironmentFile=/etc/alga/env
ExecStart=/usr/local/bin/alga teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/alga-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Alga Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now alga-teams-pipeline-maintain.timer
systemctl list-timers alga-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/alga teams-pipeline maintain-subscriptions >> /var/log/alga/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.alga/.env` at the top of a wrapper script that crontab calls.
...
```bash
alga teams-pipeline subscriptions   # should show expirationDateTime advanced
alga teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
alga teams-pipeline list
alga teams-pipeline list --status failed
alga teams-pipeline show <job-id>
```

### Replay a stored job

```bash
alga teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
alga teams-pipeline fetch --meeting-id <meeting-id>
alga teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `alga teams-pipeline maintain-subscriptions --dry-run`
- inspect `alga teams-pipeline list --status failed`
...
- run `alga teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `alga teams-pipeline run`
...
- [ ] `alga teams-pipeline validate` returns an OK snapshot
- [ ] `alga teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Alga cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
