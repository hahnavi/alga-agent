---
sidebar_position: 8
title: "Use Voice Mode with Alga"
description: "A practical guide to setting up and using Alga voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Alga
...
- you want Alga sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Alga.
...
## Step 1: make sure normal Alga works first
...
Alga supports both local and cloud speech stacks.
...
### If you use `alga setup`

If you choose NeuTTS in the setup wizard, Alga checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Alga transcribes and responds
...
- asking Alga to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Alga loop.

## Tuning CLI behavior

### Silence threshold

If Alga starts/stops too aggressively, tune:
...
Alga stays a normal chat bot, but can speak replies.
...
- you want Alga to function like a portable research or ops assistant
...
Alga joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Alga detects speech boundaries
- transcripts are posted in the associated text channel
- Alga responds in text and audio
...
1. get text Alga working
2. install `alga-agent[voice]`
