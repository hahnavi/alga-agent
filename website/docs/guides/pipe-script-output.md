...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `alga send`."
---

# Pipe Script Output to Messaging Platforms

`alga send` is a small, scriptable CLI that pushes a message to any
messaging platform Alga is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | alga send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `alga
...
alga send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
alga send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
alga send --list

# Filter by platform
alga send --list telegram
```
...
Any platform Alga ships adapters for works as a target:
...
`alga send` resolves the message body in this order:

1. **Positional argument** — `alga send --to telegram "hi"`
2. **`--file PATH`** — `alga send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | alga send --to telegram`

When stdin is a TTY (no pipe), Alga does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  alga send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `alga send` reuses your Alga config, the same script works on
any host where Alga is installed — no need to export bot tokens into
...
`alga send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  alga send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | alga send \
...
```bash
./train.py --epochs 200 && \
  alga send --to telegram "training done" || \
  alga send --to telegram "training failed (exit $?)"
```
...
alga send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(alga send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `alga send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `alga send` calls
the platform's REST endpoint directly using credentials from
`~/.alga/.env` and `~/.alga/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `alga gateway start` and retry.
...
The listing is built from `~/.alga/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`alga gateway
...
| Approach | Multi-platform | Reuses Alga creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `alga send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`alga send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `alga send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `alga send` shares with cron delivery.
