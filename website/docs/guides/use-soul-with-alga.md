---
sidebar_position: 7
title: "Use SOUL.md with Alga"
description: "How to use SOUL.md to shape Alga Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Alga

`SOUL.md` is the **primary identity** for your Alga instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Alga to feel like the same assistant every time you talk to it — or if you want to replace the Alga persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Alga should be
- what Alga should avoid stylistically
- how Alga should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Alga is and how Alga speaks
...
Alga now uses only the global SOUL file for the current instance:

```text
~/.alga/SOUL.md
```

If you run Alga with a custom home directory, it becomes:
...
Alga automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Alga does not overwrite it
- if the file exists but is empty, Alga adds nothing from it to the prompt

## How Alga uses it

When Alga starts a session, it reads `SOUL.md` from `ALGA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Alga feels.
...
Alga already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Alga should sound.

# Avoid
What Alga should not do.

# Defaults
How Alga should behave when ambiguity appears.
```
...
Then restart Alga or start a new session.
...
### I edited SOUL.md but Alga still sounds the same

Check:
- you edited `~/.alga/SOUL.md` or `$ALGA_HOME/SOUL.md`
...
### Alga is ignoring parts of my SOUL.md
