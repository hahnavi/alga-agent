...
Alga Agent 使用 SQLite 数据库（`~/.alga/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。
...
多个 alga 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
该路径由 `alga_constants.get_alga_home()` 推导，默认解析为 `~/.alga/`，或 `ALGA_HOME` 环境变量的值。
