#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Alga docs site.
...
Both publish at:
  https://alga-agent.hahnavi.my.id/docs/llms.txt
  https://alga-agent.hahnavi.my.id/docs/llms-full.txt
...
SITE_BASE = "https://alga-agent.hahnavi.my.id/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Alga as a Python Library", None),
        ("guides/use-mcp-with-alga", "Use MCP with Alga", None),
        ("guides/use-voice-mode-with-alga", "Use Voice Mode with Alga", None),
        ("guides/use-soul-with-alga", "Use SOUL.md with Alga", None),
        ("guides/build-a-alga-plugin", "Build a Alga Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Alga"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "alga-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/hahnavi/alga-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Alga Agent — Full Documentation\n",
        (
            "This file is the entire Alga Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Alga, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://alga-agent.hahnavi.my.id/docs\n",
        "Short index: https://alga-agent.hahnavi.my.id/docs/llms.txt\n",
        "\n---\n\n",
    ]
