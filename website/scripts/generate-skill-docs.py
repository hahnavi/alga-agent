...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/hahnavi/alga-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    alga_meta = (fm.get("metadata") or {}).get("alga") or {}
    tags = alga_meta.get("tags") or []
    related = alga_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `alga skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Alga Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Alga ships with a large built-in skill library copied into `~/.alga/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Alga also syncs bundled skills on `alga update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.alga/skills/` tree, it is still shipped with Alga; restore it with `alga skills reset <name> --restore`.",
        "",
