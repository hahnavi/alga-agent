...
# 5. Symlinks the 'alga' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🍀 Alga Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/alga-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink alga into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up alga command..."

ALGA_BIN="$SCRIPT_DIR/venv/bin/alga"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ALGA_BIN" "$COMMAND_LINK_DIR/alga"
echo -e "${GREEN}✓${NC} Symlinked alga → $COMMAND_LINK_DISPLAY_DIR/alga"
...
# ============================================================================
# Seed bundled skills into ~/.alga/skills/
# ============================================================================

ALGA_SKILLS_DIR="${ALGA_HOME:-$HOME/.alga}/skills"
mkdir -p "$ALGA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.alga/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ALGA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  alga status        # Check configuration"
if is_termux; then
    echo "  alga gateway       # Run gateway in foreground"
else
    echo "  alga gateway install # Install gateway service (messaging + cron)"
fi
echo "  alga cron list     # View scheduled jobs"
echo "  alga doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m alga_cli.main setup
fi
